/**
 * Tunnel de vente RETARGETING — Crème Contour des Yeux Anti-Cernes (CREME_ANTI_CERNE_OFFRE)
 * Slug : creme-anti-cerne-offre
 *
 * Clone de creme-anti-cerne pour campagnes de retargeting Meta (visiteurs sans
 * achat) : prix reduit a 8 900 F (-1 000 F), palette fuchsia/rose, CTA pilule.
 * Memes medias /creme-anti-cerne/* que la page originale (cache partage).
 */
...
const SLUG = 'creme-anti-cerne-offre';
const PRODUCT_CODE = 'CREME_ANTI_CERNE_OFFRE';
const META_PIXEL_ID = '2289664558455796';
const THANK_YOU_URL = '/creme-anti-cerne-offre/merci';

const PRICES: Record<number, number> = { 1: 8900, 2: 15900, 3: 22900 };
const fmtTotal = (qty: number) => orderTotal(PRICES, qty).toLocaleString('fr-FR').replace(/\u202f|,/g, ' ');
const QTY_OPTS = [
  { v: 1, label: '1 crème', sub: packLabel(PRICES, 1, 'FCFA') },
  { v: 2, label: '2 crèmes', sub: packLabel(PRICES, 2, 'FCFA'), tag: 'Le + choisi', save: 'Économisez 1 900 F' },
  { v: 3, label: '3 crèmes', sub: packLabel(PRICES, 3, 'FCFA'), tag: 'Meilleure offre', save: 'Économisez 3 800 F' },
];
    ...
    return (
      <div ref={ref} className={`overflow-hidden ${className}`}>
        {visible ? (
          ...
          <div className="min-h-[280px] w-full animate-pulse bg-fuchsia-50" />
        )}
      </div>
    );
  }
...
function Hot({ children }: { children: ReactNode }) {
  return (
    <span className="bg-gradient-to-r from-rose-600 via-fuchsia-500 to-orange-500 bg-clip-text font-black text-transparent">
      {children}
    </span>
  );
}
...
function FluidCTA({ onClick, children, variant = 'red' }: { onClick: () => void; children: ReactNode; variant?: 'red' | 'dark' | 'white' }) {
  const cls =
    variant === 'red'
      ? 'from-rose-600 via-fuchsia-600 to-rose-700 text-white ring-white/30'
      : variant === 'dark'
        ? 'from-neutral-950 via-neutral-800 to-neutral-950 text-white ring-fuchsia-300/30'
        : 'from-white via-fuchsia-50 to-white text-rose-700 ring-fuchsia-300';
  return (
    <button
      type="button"
      onClick={onClick}
      className={`cac-fluid cac-fluid-pulse group relative inline-flex w-full items-center justify-center gap-2 overflow-hidden rounded-full bg-gradient-to-r ${cls} px-6 py-4 text-[14px] font-black uppercase tracking-[0.18em] shadow-[0_18px_44px_-10px_rgba(244,63,94,.55)] ring-2 transition hover:scale-[1.02] sm:text-[15px]`}
    >
...
function Marquee({ items, speed = 32 }: { items: string[]; speed?: number }) {
  return (
    <div className="overflow-hidden border-y border-fuchsia-200 bg-gradient-to-r from-fuchsia-50 via-white to-fuchsia-50 py-2.5">
      <div
        className="cac-marquee flex w-[200%] items-center gap-8 text-[10px] font-black uppercase tracking-[0.28em] text-rose-700/85 sm:text-[11px]"
...
function Fiche({ kicker, hook, cta, qty, onOrder, media, variant = 'white' }: FicheProps) {
  const bg =
    variant === 'dark'
      ...
      : variant === 'beige'
        ? 'bg-gradient-to-b from-stone-100 via-[#fafaf6] to-fuchsia-50/50 text-neutral-900'
        : 'bg-white text-neutral-900';
  const kickerColor = variant === 'dark' ? 'text-fuchsia-300' : 'text-rose-600';
  const subColor = variant === 'dark' ? 'text-stone-300' : 'text-neutral-600';

  return (
    <section className={`relative overflow-hidden py-10 sm:py-14 ${bg}`}>
      <div className="pointer-events-none absolute -left-20 top-10 h-40 w-40 rounded-full bg-fuchsia-300/20 blur-3xl cac-float-slow" />
      <div className="pointer-events-none absolute -right-20 bottom-10 h-48 w-48 rounded-full bg-rose-300/15 blur-3xl cac-float-slow" style={{ animationDelay: '2.5s' }} />
      ...
      <div className="relative mx-auto max-w-xl px-4 text-center">
        ...
        <div className="relative mx-auto max-w-[460px] overflow-hidden rounded-[2rem] ring-1 ring-fuchsia-200 shadow-[0_25px_60px_-22px_rgba(244,63,94,.3)]">
          {media}
        </div>
        <div className="mt-6">
    ...
    <div className="min-h-screen bg-white text-neutral-900" style={{ fontFamily: 'system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif' }}>
      <style>{`
        ...
        @keyframes cac-fluid-pulse {
          0%, 100% { box-shadow: 0 18px 44px -10px rgba(244,63,94,.55); transform: translateY(0); }
          50% { box-shadow: 0 26px 60px -8px rgba(244,63,94,.75); transform: translateY(-2px); }
        }
        ...
        .cac-shimmer-red {
          background: linear-gradient(90deg, #e11d48 0%, #fb7185 25%, #f43f5e 50%, #fda4af 75%, #e11d48 100%);
      ...
      <div className="sticky top-0 z-50 border-b border-fuchsia-200 bg-white/95 shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-2 px-3 py-2 sm:gap-4">
          <span className="relative flex h-2 w-2 text-rose-500 cac-pulse-dot">
          ...
          <div className="flex items-center gap-1">
            {[countdown.h, countdown.m, countdown.s].map((v, i) => (
              <span key={i} className="flex items-center gap-1">
                ...
                <span className="inline-flex h-7 min-w-[32px] items-center justify-center rounded-md bg-rose-50 px-1.5 font-mono text-[13px] font-black tabular-nums text-rose-700 ring-1 ring-rose-200">
                  {pad(v)}
                </span>
              </span>
            ))}
          </div>
          <span className="inline-flex items-center gap-1 rounded-full bg-rose-600 px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider text-white">
            🔥 Stock {stock}
          </span>
        </div>
        <div className="h-[3px] w-full bg-fuchsia-50">
          <div
            className="h-full bg-gradient-to-r from-rose-500 via-fuchsia-500 to-rose-600 transition-all duration-700"
            style={{ width: `${stockPct}%` }}
          />
        </div>
      </div>

      <Marquee items={['OFFRE EXCLUSIVE -1 000 F', 'CE SOIR SEULEMENT', 'ANTI-CERNES & ANTI-RIDES', 'PAIEMENT LIVRAISON', '24H ABIDJAN', 'STOCK LIMITÉ']} />

      {/* HERO */}
      <section className="relative overflow-hidden bg-white py-6 sm:py-10">
        <div className="pointer-events-none absolute -left-16 -top-16 h-72 w-72 rounded-full bg-fuchsia-300/30 blur-3xl cac-float-slow" />
        <div className="pointer-events-none absolute -right-20 top-32 h-72 w-72 rounded-full bg-rose-300/25 blur-3xl cac-float-slow" style={{ animationDelay: '3s' }} />

        <div className="relative mx-auto max-w-3xl px-4 text-center">
          <p className="inline-flex items-center gap-2 rounded-full border border-fuchsia-200 bg-fuchsia-50/80 px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.32em] text-rose-700 backdrop-blur cac-fade-up">
            <span className="h-1 w-1 rounded-full bg-rose-500" />
            Offre retour · -1 000 F · ce soir seulement
            <span className="h-1 w-1 rounded-full bg-rose-500" />
          </p>
          ...
          <div className="relative mt-5 cac-fade-up" style={{ animationDelay: '.05s' }}>
            ...
            <div className="absolute -left-3 top-10 rotate-[-7deg] rounded-md bg-rose-700 px-3 py-2 text-center shadow-xl">
              <p className="text-[9px] font-black uppercase tracking-[0.28em] text-fuchsia-200">Soin</p>
              <p className="cac-shimmer-red text-[16px] font-black leading-tight">Premium</p>
            </div>
            <div className="absolute -right-3 bottom-12 rotate-[6deg] rounded-md bg-white px-3 py-2 shadow-xl ring-1 ring-fuchsia-200">
          ...
          <div className="mt-5 cac-fade-up" style={{ animationDelay: '.2s' }}>
            <div className="flex items-baseline justify-center gap-3">
              ...
              <span className="rounded-md bg-rose-700 px-2 py-0.5 text-[10px] font-black uppercase tracking-[0.18em] text-white">-34 %</span>
            </div>
      ...
      <section className="border-y border-fuchsia-100 bg-fuchsia-50/40">
      ...
      <section className="bg-stone-50 py-12 sm:py-14">
        <div className="mx-auto max-w-xl px-4 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.32em] text-rose-700">Le problème</p>
          ...
          <ul className="mx-auto mt-5 grid max-w-md grid-cols-2 gap-2 text-left text-[13px] font-bold text-neutral-800 sm:text-[14px]">
            {['Cernes visibles', 'Poches sous les yeux', 'Ridules autour du regard', 'Regard terne et fatigué'].map((x, i) => (
              <li key={i} className="flex items-start gap-2 rounded-xl bg-white p-3 shadow-sm ring-1 ring-fuchsia-100">
      ...
      <section className="bg-white py-12 sm:py-14">
        <div className="mx-auto max-w-xl px-4 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.32em] text-rose-700">La solution</p>
          <h2 className="mt-3 text-balance text-[24px] font-black leading-tight text-neutral-900 sm:text-[30px]">
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={i} className="flex items-center gap-3 rounded-2xl bg-gradient-to-br from-fuchsia-50 via-white to-stone-50 p-4 ring-1 ring-fuchsia-100 shadow-sm">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-rose-600 to-fuchsia-500 text-lg text-white shadow">
                  {b.ico}
                </span>
      ...
      <section className="bg-white py-12 sm:py-14">
        <div className="mx-auto max-w-3xl px-4 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.32em] text-rose-700">Vu par nos clientes</p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
            {['Avis vérifiés WhatsApp', 'Recommandé par nos clientes', 'Témoignages clients', 'Résultats observés'].map((p) => (
              <span key={p} className="rounded-md border border-fuchsia-200 bg-fuchsia-50/40 px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-rose-700 shadow-sm">{p}</span>
            ))}
          </div>
          <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-neutral-950 px-4 py-2 text-white shadow-xl">
            ...
            <span className="text-[10px] uppercase tracking-wider text-fuchsia-300">2 800+ clientes</span>
          </div>
        </div>
      </section>
      ...
      <section className="bg-stone-50 py-12 sm:py-14">
        <div className="mx-auto max-w-xl px-4">
          <p className="text-center text-[10px] font-black uppercase tracking-[0.32em] text-rose-700">Galerie</p>
          ...
          <div className="relative mt-6 overflow-hidden rounded-[2rem] bg-white shadow-xl ring-1 ring-fuchsia-200">
            <div className="relative aspect-[4/5] sm:aspect-[16/10]">
              ...
              <button
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-rose-700 shadow-lg ring-1 ring-fuchsia-200 transition hover:bg-white"
              >
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-rose-700 shadow-lg ring-1 ring-fuchsia-200 transition hover:bg-white"
              >
            ...
            <div className="flex justify-center gap-2 py-3">
              {galleryImgs.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${i === galleryIdx ? 'w-8 bg-rose-600' : 'w-2 bg-fuchsia-200 hover:bg-fuchsia-300'}`}
                />
              ))}
            </div>
          </div>
      ...
      <section className="bg-[#e5ddd5] py-12 sm:py-14">
        <div className="mx-auto max-w-md px-4">
          <p className="text-center text-[10px] font-black uppercase tracking-[0.32em] text-rose-800">Avis vérifiés WhatsApp</p>
          ...
          <div className="mt-5 overflow-hidden rounded-2xl shadow-2xl">
            <div className="flex items-center gap-3 bg-[#075e54] px-4 py-3 text-white">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-rose-500 to-fuchsia-500 font-black text-white">GS</div>
            ...
            <div className="space-y-2 bg-[#ece5dd] px-3 py-4">
              {reviews.map((r, i) => (
                <div key={i} className="max-w-[85%] rounded-2xl rounded-tl-sm bg-white px-3 py-2 shadow-sm">
                  <p className="text-[11px] font-black text-rose-700">{r.name} · {r.city}</p>
      ...
      <section className="bg-white py-12 sm:py-14">
        <div className="mx-auto max-w-xl px-4 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.32em] text-rose-700">Pourquoi un soin ciblé ?</p>
          ...
          <ul className="mx-auto mt-5 grid max-w-md grid-cols-2 gap-2 text-left text-[12px] font-bold text-neutral-800 sm:text-[13px]">
            {['Texture légère', 'Application facile', 'Soin ciblé contour', 'Routine simple'].map((x, i) => (
              <li key={i} className="flex items-start gap-2 rounded-xl bg-fuchsia-50/60 p-3 ring-1 ring-fuchsia-100">
                <span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-rose-600 text-white shadow">
      ...
      <section className="bg-stone-50 py-12 sm:py-14">
        <div className="mx-auto max-w-xl px-4">
          <p className="text-center text-[10px] font-black uppercase tracking-[0.32em] text-rose-700">Mode d'emploi</p>
          <h2 className="mt-3 text-center text-balance text-[22px] font-black leading-tight text-neutral-900 sm:text-[28px]">
          ...
          <ol className="mt-6 space-y-3">
            {[
              ...
              <li key={s.n} className="flex gap-3 rounded-2xl bg-white p-4 ring-1 ring-fuchsia-100 shadow-sm">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-rose-600 to-fuchsia-500 text-[14px] font-black text-white shadow">
                  {s.n}
                </div>
      ...
      <section className="bg-white py-12 sm:py-14">
        <div className="mx-auto max-w-2xl px-4">
          <div className="text-center">
            <span className="rounded-full bg-fuchsia-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-rose-700">Offre spéciale</span>
            <h2 className="mt-3 text-balance text-[22px] font-black leading-tight text-neutral-900 sm:text-[28px]">
            ...
            {[
              ...
              <button
                ...
                className={`group relative overflow-hidden rounded-2xl border-2 p-5 text-left transition hover:scale-[1.02] hover:shadow-xl ${
                  b.hot
                    ? 'border-rose-500 bg-gradient-to-br from-fuchsia-50 via-white to-fuchsia-50 shadow-lg ring-2 ring-rose-300/40'
                    : 'border-fuchsia-200 bg-gradient-to-br from-white to-stone-50'
                }`}
              >
                {b.hot && (
                  <span className="absolute -top-1 right-4 rotate-3 rounded-b-md bg-rose-600 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-white shadow">★ POPULAIRE</span>
                )}
                <p className="text-[10px] font-black uppercase tracking-wider text-rose-700">{b.sub}</p>
                ...
                {b.save && (
                  <p className="mt-2 inline-flex rounded-full bg-rose-700 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-white">{b.save}</p>
                )}
              </button>
            ))}
          </div>
          ...
          {/* Upsell */}
          <div className="mt-6 flex flex-col items-center gap-4 rounded-3xl border-2 border-dashed border-rose-300 bg-gradient-to-br from-fuchsia-50 to-white p-5 text-center sm:flex-row sm:text-left">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-rose-700 text-3xl text-white shadow-lg">🎁</div>
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase tracking-wider text-rose-700">Bonus offert</p>
            ...
            <button
              type="button"
              onClick={() => openModal(3)}
              className="cac-fluid relative inline-flex shrink-0 items-center gap-2 overflow-hidden rounded-full bg-gradient-to-r from-rose-600 to-fuchsia-600 px-4 py-2.5 text-[12px] font-black uppercase tracking-wider text-white shadow-lg ring-2 ring-white/20"
            >
              <span className="cac-sheen pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/45 to-transparent" />
      ...
      <section className="bg-stone-50 py-12">
        <div className="mx-auto max-w-2xl px-4">
          <div className="rounded-3xl bg-gradient-to-br from-rose-700 via-fuchsia-700 to-rose-800 p-7 text-white shadow-2xl">
            <p className="text-[10px] font-black uppercase tracking-[0.32em] text-fuchsia-200">Notre engagement</p>
            ...
            <ul className="mt-5 space-y-2 text-[14px]">
              {[
                ...
                <li key={i} className="flex items-start gap-3">
                  <span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white text-rose-700 shadow">
                    <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>
                  </span>
                  <span>{x}</span>
                </li>
        ...
        <div className="mx-auto max-w-2xl px-4">
          ...
          <div className="mt-6 space-y-3">
            {[
              ...
              <details key={i} className="group overflow-hidden rounded-2xl bg-stone-50 shadow-sm ring-1 ring-fuchsia-100 transition-all open:ring-fuchsia-300 open:bg-white">
                <summary className="flex cursor-pointer items-center justify-between px-5 py-4 text-[14px] font-black text-neutral-900 sm:text-[15px]">
                  <span>{f.q}</span>
                  <svg className="h-5 w-5 text-rose-600 transition-transform duration-300 group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.6}>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-rose-950/95 via-rose-900/85 to-fuchsia-900/85" />
        <div className="pointer-events-none absolute -left-12 top-10 h-40 w-40 rounded-full bg-fuchsia-400/30 blur-3xl cac-float-slow" />
        <div className="pointer-events-none absolute -right-16 bottom-12 h-48 w-48 rounded-full bg-rose-300/20 blur-3xl cac-float-slow" style={{ animationDelay: '2s' }} />

        <div className="relative z-10 mx-auto max-w-xl px-4 py-16 text-center text-white sm:py-20">
          <p className="text-[10px] font-black uppercase tracking-[0.36em] text-fuchsia-300">Dernière ligne droite</p>
          ...
          <p className="mt-3 text-[13px] font-semibold text-fuchsia-100/95 sm:text-[14px]">Compte à rebours actif · stock affiché en temps réel.</p>

          <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 ring-1 ring-fuchsia-300/40 backdrop-blur">
            <span className="text-[10px] font-black uppercase tracking-wider text-fuchsia-200">Fin promo</span>
          ...
          <div className="mx-auto mt-7 max-w-sm">
            ...
            <p className="mt-3 text-[11px] text-fuchsia-200">Sans engagement · paiement à la réception</p>
          </div>
        </div>
      </section>

      {/* CARROUSEL TEMOIGNAGES (4 avis dots) */}
      <section className="bg-fuchsia-50/40 py-12 sm:py-14">
        <div className="mx-auto max-w-xl px-4">
          <div className="text-center">
            <span className="rounded-full bg-fuchsia-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-rose-700">Témoignages clients</span>
            <h2 className="mt-3 text-balance text-[22px] font-black leading-tight text-neutral-900 sm:text-[28px]">
              <Hot>4,9/5</Hot> · des centaines de retours
            </h2>
          </div>

          <div className="relative mt-6 min-h-[210px] overflow-hidden rounded-3xl bg-white p-6 ring-1 ring-fuchsia-200 shadow-xl">
            {reviews.map((r, i) => (
              <div
                key={r.name}
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-rose-700">{r.city}</p>
                  </div>
                  <span className="flex">{Array.from({length: r.stars}).map((_, j) => <Star key={j}/>)}</span>
                </div>
                <p className="mt-3 text-[15px] font-semibold leading-relaxed text-neutral-800">"{r.txt}"</p>
                <p className="mt-3 text-right text-[10px] text-rose-600">{r.t} · ✓ Vérifié WhatsApp</p>
              </div>
            ))}
            <div className="mt-[190px] flex justify-center gap-2">
              {reviews.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${i === carouselIdx ? 'w-8 bg-rose-600' : 'w-2 bg-fuchsia-200 hover:bg-fuchsia-300'}`}
                />
              ))}
            </div>
          </div>
      ...
      <footer className="bg-neutral-950 py-7 pb-24 text-center text-[10px] font-semibold text-fuchsia-300/70 sm:pb-7">
      ...
      {toast && !modal && (
        ...
        >
          <div className="flex items-center gap-2.5 rounded-xl border border-fuchsia-200 bg-white px-3 py-2.5 shadow-[0_12px_30px_-6px_rgba(244,63,94,.25)]">
            <span className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-rose-500 via-fuchsia-500 to-rose-600 text-white font-black ring-2 ring-fuchsia-100">
              <span className="absolute inset-0 animate-ping rounded-full bg-fuchsia-400/50" />
              <span className="relative text-[12px]">{toast.n[0]}</span>
            </span>
            <div className="min-w-0 flex-1 leading-tight">
              <p className="text-[11px] font-black text-neutral-900">
                <span className="text-rose-700">{toast.n}</span> · {toast.v}
              </p>
      ...
      <div
        className={`fixed inset-x-0 bottom-0 z-40 border-t border-fuchsia-200 bg-white/97 px-3 py-2.5 backdrop-blur-md sm:hidden ${
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={() => openModal(1)}
            className="cac-fluid cac-fluid-pulse relative inline-flex items-center gap-1.5 overflow-hidden rounded-full bg-gradient-to-r from-rose-600 via-fuchsia-600 to-rose-700 px-5 py-2.5 text-[13px] font-black uppercase tracking-[0.16em] text-white shadow-[0_10px_24px_-4px_rgba(244,63,94,.55)] ring-2 ring-white/30"
          >
            <span className="cac-sheen pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/45 to-transparent" />
