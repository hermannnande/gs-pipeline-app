/**
 * Page de remerciement dediee a la campagne TikTok patchdouleurtiktok.
 *
 * Role principal : declencher un evenement d'achat au chargement pour permettre
 * l'optimisation de la campagne. Pour l'instant AUCUN pixel n'est configure
 * (META_PIXEL_ID='') : la logique de tracking reste en place mais ne fait rien.
 * Pour activer TikTok (ttq CompletePayment) ou Meta plus tard, renseigner l'ID.
 *
 * Securite anti-double-fire :
 *   - eventID base sur orderReference (?ref=) -> permet la deduplication.
 *   - sessionStorage marque le ref comme deja track pour eviter un re-fire.
 */
import { useEffect, useState } from 'react';
import { Link, useSearchParams } from 'react-router-dom';

// Pixel de la campagne TikTok. AUCUN pour l'instant (a renseigner plus tard).
const META_PIXEL_ID = '';
...
export default function PatchDouleurTiktokThankYou() {
  ...
  // ============= TRACKING (inactif tant que META_PIXEL_ID est vide) =============
  useEffect(() => {
    initMetaPixel(META_PIXEL_ID);

    // Anti-double-fire : si on a deja track ce ref dans cette session, skip.
    const sessionKey = ref ? `pd_tt_purchase_${ref}` : '';
    ...
    const fire = () => {
      try {
        ...
        console.warn('[thankyou] Pixel Purchase non bloquant:', err);
      }
    };

    // Si fbq pas encore charge, on retente une fois (~800ms apres le script init)
    if (window.fbq) fire();
  ...
  return (
    <div className="pdtt-tx-root min-h-screen overflow-x-hidden bg-[#fafaf9] text-[#0a1628] antialiased">
        ...
        <div className="relative mx-auto w-full max-w-[640px]">
          ...
          <div className="relative overflow-hidden rounded-[32px] bg-white p-6 shadow-[0_30px_80px_-20px_rgba(6,182,212,0.35)] ring-1 ring-cyan-200/60 sm:p-10">
            ...
            <div className="text-center">
              ...
              <h1 className="pdtt-display mt-4 text-[30px] font-black leading-[1.05] tracking-tight text-[#0a1628] sm:text-[38px]">
                Merci pour votre <span className="pdtt-grad-cyan">commande</span> !
              </h1>
            ...
            <div className="mt-6 rounded-2xl bg-gradient-to-br from-[#0a1628] via-[#0c1e2e] to-[#0a1628] p-4 text-white ring-1 ring-cyan-400/20">
              <div className="grid grid-cols-2 gap-3 text-[12px]">
                ...
                <div className="text-right">
                  ...
                  <p className="pdtt-display mt-1 text-[18px] font-black tabular-nums text-cyan-300">{value.toLocaleString('fr-FR').replace(/,/g, ' ')} F</p>
                </div>
              </div>
              {ref && (
      ...
      <style>{`
        .pdtt-display    { font-family: 'Bricolage Grotesque', 'Outfit', system-ui, -apple-system, sans-serif; font-feature-settings: 'ss01' on, 'ss02' on; }
        .pdtt-grad-cyan  { background: linear-gradient(135deg, #06b6d4 0%, #22d3ee 50%, #0ea5e9 100%); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; }
      `}</style>
    </div>
  );
}
