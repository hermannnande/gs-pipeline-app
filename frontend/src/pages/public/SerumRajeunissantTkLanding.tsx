/**
 * Landing premium — Sérum Rajeunissant Anti-Âge Yeux (SERUM_RAJEUNISSANT_TK).
 * Variante TIKTOK (slug serum-rajeunissant-tk) : meme design, stats separees,
 * produit dedie dans obgestion. Pixel Meta desactive — Pixel TikTok : renseigner TIKTOK_PIXEL_ID.
...
const PRODUCT_CODE = 'SERUM_RAJEUNISSANT_TK'; // Produit dedie : commandes TikTok separees dans obgestion
...
const THANK_YOU_URL = '/serum-rajeunissant-tk/merci';
      ...
      <div className="border-t border-white/25 bg-gradient-to-r from-[#7B4B2A]/95 via-[#E8739E]/95 to-[#A855F7]/95 shadow-[0_-10px_34px_-10px_rgba(62,36,21,.45)] backdrop-blur-md">
        <div className="mx-auto flex max-w-lg items-center gap-3 px-4 py-2.5">
          ...
          <button
            type="button"
            onClick={onClick}
            className="sr-cta-btn shrink-0 rounded-xl bg-white px-5 py-2.5 text-[12.5px] font-black uppercase tracking-[0.1em] text-[#7B4B2A] shadow-lg transition hover:scale-[1.03] active:scale-[0.98]"
          >
            Commander ✨
          </button>
        </div>
...
/* Landing                                                             */
/* ================================================================== */
export default function SerumRajeunissantTkLanding() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FFF6EF] via-[#FDF0F5] to-[#F3E8FF] pb-28 text-neutral-900">
      <style>{`
        ...
        @keyframes sr-bounce { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-6px)} }
        @keyframes sr-shine { 0%{left:-60%} 55%{left:120%} 100%{left:120%} }
        .sr-cta-btn { position:relative; overflow:hidden; animation: sr-bounce 1.4s ease-in-out infinite; }
        .sr-cta-btn::after { content:''; position:absolute; top:0; left:-60%; height:100%; width:40%; transform:skewX(-20deg); background:linear-gradient(90deg,transparent,rgba(255,255,255,.75),transparent); animation:sr-shine 1.8s ease-in-out infinite; }
        @media (prefers-reduced-motion: reduce) { .sr-marquee, .sr-cta-btn, .sr-cta-btn::after { animation: none; } }
      `}</style>
