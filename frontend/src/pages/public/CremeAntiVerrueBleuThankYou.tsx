/**
 * Page de remerciement dediee a /creme-anti-verrue-bleu (soindemoi.net, etc.)
 * Pixels Meta :
 *   - 1417398840151713 (historique)
 *   - 997153262936532 (campagne — Purchase via trackSingle + CAPI)
 *   - PageView au mount
 *   - Purchase avec eventID = `purchase_<ref>` (deduplication CAPI server-side)
 *   - sessionStorage `cavb_purchase_<ref>` pour eviter le re-fire au refresh
...
const TEMPLATE_SLUG = 'creme-anti-verrue-bleu';
const PRODUCT_CODE = 'CREME_ANTI_VERRUE_BLEU';
const PRODUCT_NAME = 'Crème Anti-Verrues';
...
export default function CremeAntiVerrueBleuThankYou() {
  ...
  useEffect(() => {
    ...
    const sessionKey = ref ? `cavb_purchase_${ref}` : '';
    ...
    const firePurchase = () => {
      try {
        ...
        firePurchaseOnPixels(META_PIXEL_IDS, {
        ...
        console.warn('[CremeAntiVerrueBleuThankYou] Purchase non bloquant:', e);
      }
    };
        ...
        <div className="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-[0_20px_60px_-12px_rgba(37,99,235,.25)]">
          ...
          <div className="space-y-4 p-5">
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <Link
                to={`/creme-anti-verrue-bleu?company=${encodeURIComponent(company)}`}
