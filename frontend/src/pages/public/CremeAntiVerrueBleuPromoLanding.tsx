/**
 * Tunnel de vente PROMO — Crème Anti-Verrues bleue (SMS / WhatsApp).
 * Slug : promo-verrue · productCode : CREME_ANTI_VERRUE_BLEU_PROMO
 * Duplication PROMO de creme-anti-verrue-bleu · prix cassé 6 500 F · SANS pixel Meta.
 *
 * Améliorations conversion vs page d'origine :
 *   - Badge -57% + économie affichée en clair (hero + barre fixe)
 *   - Barre de réassurance (4 garanties) juste sous le hero
 *   - Section "Comment l'utiliser" (3 étapes) pour lever le doute d'usage
 *   - Bloc GARANTIE satisfait ou remboursé
 *   - FAQ (traitement des objections) en accordéon
...
const PRODUCT_CODE = 'CREME_ANTI_VERRUE_BLEU_PROMO';
const CONTENT_NAME = 'Crème Anti-Verrues';
const THANK_YOU_URL = '/promo-verrue/merci';
const META_PIXEL_ID = ''; // page PROMO SMS/WhatsApp : aucun pixel Facebook

const PRICES: Record<number, number> = { 1: 6500, 2: 12000, 3: 16500 };
const OLD_UNIT = 15000;
const DISCOUNT_PCT = Math.round((1 - PRICES[1] / OLD_UNIT) * 100); // -57%
const SAVE_UNIT = OLD_UNIT - PRICES[1]; // 8 500 F
const fmt = (n: number) => n.toLocaleString('fr-FR').replace(/ |,/g, ' ');
const fmtPack = (v: number) => packLabel(PRICES, v, 'F').replace(' F', '');
const QTY_OPTS = [
  { v: 1, label: '1 boîte', sub: packLabel(PRICES, 1, 'F'), save: '' },
  { v: 2, label: '2 boîtes', sub: packLabel(PRICES, 2, 'F'), tag: 'Populaire', save: 'Économisez 1 000 F' },
  { v: 3, label: '3 boîtes', sub: packLabel(PRICES, 3, 'F'), tag: 'Meilleure offre', save: 'Économisez 3 000 F' },
];
...
const STEPS = [
  { n: '1', t: 'Nettoyez', d: 'Lavez et séchez la zone concernée.' },
  { n: '2', t: 'Appliquez', d: 'Déposez la crème sur la verrue, matin et soir.' },
  { n: '3', t: 'Résultat', d: 'La verrue sèche et disparaît en quelques jours.' },
];

const FAQ = [
  { q: 'La crème fait-elle mal ?', a: 'Non. La formule est douce, sans brûlure ni douleur. Elle agit en profondeur sans abîmer la peau autour.' },
  { q: 'En combien de temps voit-on un résultat ?', a: 'La plupart des clients voient la verrue sécher et se détacher en quelques jours à 2 semaines selon sa taille.' },
  { q: 'Ça marche sur toutes les verrues ?', a: 'Oui : mains, pieds, doigts et corps. Pour les verrues très anciennes, le pack 2 ou 3 boîtes est conseillé.' },
  { q: 'Comment je paie ?', a: 'Vous payez uniquement à la livraison (cash), une fois le colis reçu. Aucun paiement à l’avance.' },
];
...
export default function CremeAntiVerrueBleuPromoLanding() {
  ...
  useEffect(() => {
    document.title = 'Crème Anti-Verrues — Promo -57% · 6 500 F';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-indigo-50/40 pb-28">
      <style>{`
        ...
        .av-pulse-badge { animation:av-pulse 1.6s ease-in-out infinite; }
        @keyframes av-pulse { 0%,100%{transform:scale(1)} 50%{transform:scale(1.08)} }
        @media (prefers-reduced-motion: reduce){ .av-reveal,.av-marquee,.av-animated-bg,.av-bounce,.av-pulse-badge,.av-grad-sky,.av-grad-indigo,.av-grad-cyan,.av-grad-mix{animation:none;opacity:1;transform:none} }
      `}</style>

      {/* Barre défilante haut */}
      <div className="sticky top-0 z-40 bg-indigo-950">
        <Marquee items={[
          '🔥 Promo -57%', `1 boîte ${fmtPack(1)} F`, `2 boîtes ${fmtPack(2)} F`, `3 boîtes ${fmtPack(3)} F`,
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-indigo-900 via-blue-800 to-cyan-900 px-4 pt-8 pb-12 text-white">
        ...
        <div className="relative z-10 mx-auto max-w-[560px] text-center">
          <span className="inline-flex items-center gap-2 rounded-full bg-cyan-400 px-3 py-1 text-[10px] font-black uppercase text-indigo-950">
            🔥 Promo du jour
            <span className="rounded-full bg-indigo-950 px-2 py-0.5 text-cyan-300">-{DISCOUNT_PCT}%</span>
          </span>
          <Countdown />
          ...
          <p className="mt-2 inline-block rounded-full bg-emerald-500/20 px-3 py-1 text-[12px] font-bold text-emerald-300 ring-1 ring-emerald-400/30">
            💰 Vous économisez {fmt(SAVE_UNIT)} F aujourd&apos;hui
          </p>
          <div className="mt-6 flex justify-center"><BounceCTA tone="cyan" onClick={() => openModal()}>Commander — {fmt(orderTotal(PRICES, 1))} F</BounceCTA></div>
          <p className="mt-3 text-[11px] text-sky-200/80">🔒 Paiement à la livraison · Sans risque</p>
        </div>
      </section>

      {/* Barre de réassurance (garanties) */}
      <section className="border-b border-sky-100 bg-white px-4 py-5">
        <div className="mx-auto grid max-w-[560px] grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { d: 'M5 8h14M5 8a2 2 0 100-4h6a2 2 0 012 2v2M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8', t: 'Livraison 24-48h' },
            { d: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2z', t: 'Paiement à la livraison' },
            { d: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', t: 'Satisfait ou remboursé' },
            { d: 'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z', t: 'Sans douleur' },
          ].map((r, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5 text-center">
              <ReassureIcon d={r.d} />
              <p className="text-[11px] font-bold leading-tight text-neutral-700">{r.t}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-white px-4 py-12">
        <Reveal className="mx-auto max-w-[560px]">
          <h2 className="text-center text-[24px] font-black">Comment l&apos;<GradText v="cyan">utiliser</GradText> ?</h2>
          <p className="mt-1 text-center text-[13px] font-semibold text-neutral-500">3 gestes simples, matin et soir.</p>
          <div className="mt-6 space-y-3">
            {STEPS.map((s) => (
              <div key={s.n} className="flex items-center gap-4 rounded-2xl border border-sky-100 bg-gradient-to-r from-sky-50 to-white p-4 shadow-sm">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-sky-400 to-cyan-500 text-lg font-black text-white shadow-md">{s.n}</span>
                <div>
                  <p className="text-[15px] font-black text-neutral-800">{s.t}</p>
                  <p className="text-[13px] text-neutral-600">{s.d}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="px-4 py-10">
        <Reveal className="mx-auto max-w-[560px]">
          <div className="relative overflow-hidden rounded-3xl border-2 border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-6 text-center shadow-sm">
            <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500 shadow-[0_0_25px_rgba(16,185,129,.4)]">
              <svg className="h-8 w-8 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            </div>
            <h3 className="text-[18px] font-black text-emerald-800">Satisfait ou remboursé</h3>
            <p className="mt-1.5 text-[13px] text-emerald-700">Vous ne payez qu&apos;à la réception. Si le produit ne vous convient pas, on vous rembourse. <span className="font-bold">Zéro risque.</span></p>
          </div>
        </Reveal>
      </section>
      ...
      <section className="bg-white px-4 py-12">
        <Reveal className="mx-auto max-w-[560px]">
          <h2 className="text-center text-[24px] font-black">Questions <GradText v="cyan">fréquentes</GradText></h2>
          <div className="mt-6 space-y-3">
            {FAQ.map((f, i) => (
              <details key={i} className="group rounded-2xl border border-sky-100 bg-sky-50/40 p-4">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-[14px] font-bold text-neutral-800">
                  {f.q}
          ...
          <div className="mt-7 flex justify-center"><BounceCTA tone="cyan" onClick={() => openModal()}>Commander — {fmt(orderTotal(PRICES, 1))} F</BounceCTA></div>
        </Reveal>
      </section>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 border-t border-sky-400/30 bg-indigo-950 px-4 py-3" style={{ paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom, 0px))' }}>
        <div className="mx-auto flex max-w-[560px] items-center gap-3">
          <div className="pointer-events-none min-w-0 flex-1">
            <p className="text-[10px] font-bold uppercase text-cyan-400">🔥 Promo -{DISCOUNT_PCT}% · <Countdown compact /></p>
          ...
          <button type="button" onClick={() => openModal(1)}
            className="av-pulse-badge pointer-events-auto shrink-0 rounded-2xl bg-gradient-to-r from-sky-400 via-cyan-400 to-blue-500 px-5 py-3.5 text-[12px] font-black uppercase text-white shadow-lg">
            Commander
          </button>
        </div>
      </div>
