/**
 * Page merci — Support Téléphone Flexible Mains Libres, variante TIKTOK (slug support-telephone-flexible-tk).
...
const SLUG = 'support-telephone-flexible-tk';
const PRODUCT_CODE = 'SUPPORT_TELEPHONE_FLEXIBLE_TK'; // Produit dedie (meme mapping que la landing -tk)
...
export default function SupportTelephoneFlexibleTkThankYou() {
  ...
  useEffect(() => {
    ...
    if (TIKTOK_PIXEL_ID && window.ttq) {
      window.ttq.track('CompletePayment', { content_name: 'Support Téléphone Flexible Mains Libres', content_id: PRODUCT_CODE, content_type: 'product', value: orderTotal(PRICES, qty), currency: 'XOF', quantity: qty });
      if (sk) sessionStorage.setItem(sk, '1');
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#ECFEFF] via-[#EFF6FF] to-[#F5F3FF] px-4 py-10">
      <div className="w-full max-w-md overflow-hidden rounded-[28px] border border-[#67E8F9]/40 bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-5">
          ...
          <Link to="/support-telephone-flexible-tk" className="block rounded-full bg-gradient-to-r from-[#06B6D4] via-[#3B82F6] to-[#8B5CF6] py-3.5 text-center text-[12px] font-black uppercase tracking-[0.12em] text-white shadow-lg transition hover:brightness-110">
            Retour au produit
          </Link>
        </div>
      </div>
