/**
 * Page merci — Patch Minceur GLP (slug patch-minceur-glp).
 * Pixel Meta : 924252450279192
 */
...
const TEMPLATE_SLUG = 'patch-minceur-glp';
...
export default function PatchMinceurGlpThankYou() {
  ...
  useEffect(() => { document.title = 'Merci — Patch Minceur GLP'; }, []);

  useEffect(() => {
    if (fired.current) return;
    fired.current = true;
    const sk = ref ? `pmg_purchase_${ref}` : '';
    if (sk && sessionStorage.getItem(sk)) return;
    const eventId = ref ? `purchase_${ref}` : `purchase_${Date.now()}`;
    const fire = () => {
      initPixel(META_PIXEL_ID);
      window.fbq?.('track', 'Purchase', {
        content_name: 'Patch Minceur GLP', content_ids: [PRODUCT_CODE], content_type: 'product',
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-violet-50 via-white to-fuchsia-50 px-4 py-10">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-violet-100 bg-white shadow-xl">
        <div className="bg-gradient-to-br from-violet-700 via-fuchsia-600 to-purple-700 px-6 py-8 text-center text-white">
          ...
          <p className="mt-1.5 text-[13px] text-violet-100">Votre patch minceur GLP est bien enregistré.</p>
        </div>
        <div className="space-y-4 p-5">
          ...
          <Link to="/patch-minceur-glp" className="block rounded-full bg-violet-600 py-3 text-center text-[12px] font-black uppercase text-white">Retour au produit</Link>
        </div>
      </div>
    </div>
  );
