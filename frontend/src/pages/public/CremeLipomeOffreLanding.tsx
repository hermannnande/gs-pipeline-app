/**
 * Tunnel de vente RETARGETING — Crème Anti-Lipome (CREME_LIPOME_OFFRE)
 * Slug: creme-lipome-offre
 *
 * Clone de creme-anti-lipome destine aux campagnes de retargeting Meta
 * (visiteurs sans achat) : prix reduit a 8 900 F (-1 000 F), palette violette.
 * Memes medias /lipome/n*.webp (creme bleue) que la page originale.
 *
 * Palette : violet profond · blanc · rouge corail (accent urgence).
 */
...
const SLUG = 'creme-lipome-offre';
const PRODUCT_CODE = 'CREME_LIPOME_OFFRE';
...
const META_PIXEL_ID = typeof window !== 'undefined' && window.location.hostname.includes('soindemoi')
  ? '902265788982876'
  : '1857129471642967';
const THANK_YOU_URL = '/creme-lipome-offre/merci';
...
const QTY_OPTS = [
  ...
  { v: 2, label: '2 tubes', sub: packLabel(PRICES, 2, 'FCFA'), tag: 'Le plus choisi', save: 'Économisez 1 900 F' },
  { v: 3, label: '3 tubes', sub: packLabel(PRICES, 3, 'FCFA'), tag: 'Meilleure offre', save: 'Économisez 3 800 F' },
];
...
function LazyVideo({ src, poster }: { src: string; poster?: string }) {
  const { ref, visible } = useOnScreen();
  return (
    <div ref={ref} className="relative w-full overflow-hidden bg-[#1e1145]" style={{ aspectRatio: '9/16' }}>
      {visible ? (
      ...
      <span className="pointer-events-none absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#1e1145]/90 to-transparent" />
...
function Hot({ children, red }: { children: ReactNode; red?: boolean }) {
  return (
    <span className={red
      ...
      : 'bg-gradient-to-r from-[#6d28d9] via-[#7c3aed] to-[#a78bfa] bg-clip-text font-black text-transparent'
    }>
      {children}
    </span>
  );
...
function FluidCTA({ onClick, children }: { onClick: () => void; children: ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="cal-cta cal-shake group relative inline-flex w-full items-center justify-center gap-2 overflow-hidden rounded-full bg-gradient-to-r from-[#6d28d9] via-[#7c3aed] to-[#c026d3] px-6 py-4 text-[15px] font-black text-white shadow-[0_16px_44px_-10px_rgba(124,58,237,.6)] ring-2 ring-white/30 transition hover:scale-[1.015] hover:shadow-[0_20px_50px_-8px_rgba(192,38,211,.5)] active:scale-[0.99] sm:text-[16px]"
    >
...
function Marquee({ items }: { items: string[] }) {
  return (
    <div className="overflow-hidden border-y border-[#8b5cf6]/25 bg-gradient-to-r from-[#1e1145] via-[#1e3a8a] to-[#1e1145] py-2.5">
      <div className="cal-marquee flex w-[200%] items-center gap-8 text-[10px] font-black uppercase tracking-[0.2em] text-[#ede9fe] sm:text-[11px]">
        {[0, 1].map((k) => (
...
function Fiche({ kicker, hook, cta, qty, onOrder, media, variant = 'light', shape = 'card' }: FicheProps) {
  const bg =
    variant === 'dark'
      ? 'bg-gradient-to-b from-[#1e1145] via-[#1e3a8a] to-[#1e1145] text-white'
      : variant === 'soft'
        ? 'bg-gradient-to-b from-[#f5f3ff] via-white to-[#f1f5f9] text-[#1e1145]'
        : 'bg-gradient-to-b from-[#f8fafc] via-white to-[#f5f3ff] text-[#1e1145]';

  const mediaWrap =
    shape === 'tilt'
      ? 'relative mx-auto max-w-[440px] -rotate-[1.2deg] overflow-hidden rounded-[1.75rem] shadow-[0_24px_60px_-18px_rgba(124,58,237,.35)] ring-1 ring-[#8b5cf6]/30'
      : 'relative mx-auto max-w-[440px] overflow-hidden rounded-[1.75rem] shadow-[0_24px_60px_-18px_rgba(124,58,237,.3)] ring-1 ring-[#8b5cf6]/25';

  return (
    <section className={`relative overflow-hidden py-11 sm:py-14 ${bg}`}>
      <div className="pointer-events-none absolute -left-20 top-8 h-48 w-48 rounded-full bg-[#a78bfa]/25 blur-3xl cal-float" />
      <div className="relative mx-auto max-w-xl px-4 text-center">
        {kicker && (
          <p className={`mb-2.5 text-[10px] font-black uppercase tracking-[0.28em] ${variant === 'dark' ? 'text-[#ddd6fe]' : 'text-[#7c3aed]'}`}>
            {kicker}
          </p>
        )}
        ...
        <div className="mt-6">
          ...
          <p className={`mt-2 text-[11px] font-semibold ${variant === 'dark' ? 'text-[#ddd6fe]/80' : 'text-[#64748b]'}`}>
...
export default function CremeAntiLipomeLanding() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]" style={{ fontFamily: 'system-ui,-apple-system,Segoe UI,Roboto,sans-serif' }}>
      ...
      {/* Sticky header */}
      <div className="sticky top-0 z-50 border-b border-[#8b5cf6]/30 bg-gradient-to-r from-[#1e1145]/98 via-[#1e3a8a]/95 to-[#1e1145]/98 shadow-lg backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-2 px-3 py-2 sm:gap-3">
          <span className="relative flex h-2 w-2 text-[#c4b5fd] cal-pulse-dot">
            <span className="relative inline-flex h-2 w-2 rounded-full bg-[#c4b5fd]" />
          </span>
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#ede9fe]">
            <span className="cal-shine">Offre limitée</span> · fin à minuit
          </span>
          <div className="flex items-center gap-1">
            {[countdown.h, countdown.m, countdown.s].map((v, i) => (
              <span key={i} className="flex items-center gap-1">
                {i > 0 && <span className="text-[#8b5cf6]">:</span>}
                <span className="inline-flex h-7 min-w-[30px] items-center justify-center rounded-md bg-[#8b5cf6]/25 px-1 font-mono text-[12px] font-black tabular-nums text-white ring-1 ring-[#c4b5fd]/30">
                  {pad(v)}
                </span>
              </span>
            ))}
        ...
        <div className="h-[2px] w-full bg-[#0f2a52]">
          <div className="h-full bg-gradient-to-r from-[#dc2626] via-[#a78bfa] to-[#7c3aed] transition-all duration-700" style={{ width: `${stockPct}%` }} />
        </div>
      </div>

      <Marquee items={['OFFRE EXCLUSIVE -1 000 F', 'CE SOIR SEULEMENT', 'PAIEMENT À LA LIVRAISON', 'RÉSULTATS VISIBLES 2-4 SEM', 'EXPRESS 24H']} />

      {/* Hero */}
      <section className="relative overflow-hidden bg-gradient-to-b from-[#f5f3ff] via-white to-[#f1f5f9] py-10 sm:py-14">
        <div className="pointer-events-none absolute -left-16 -top-12 h-64 w-64 rounded-full bg-[#a78bfa]/35 blur-3xl cal-float" />
        ...
        <div className="relative mx-auto max-w-xl px-4 text-center">
          <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#7c3aed]/25 bg-white/90 px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.24em] text-[#6d28d9] cal-fade-up">
            Offre retour · -1 000 F réservés à nos visiteurs · ce soir seulement
          </p>

          <h1 className="mt-3 text-balance text-[34px] font-black leading-[1.04] tracking-tight text-[#1e1145] sm:text-[46px] cal-fade-up" style={{ animationDelay: '.04s' }}>
          ...
          <div className="relative mt-7 cal-fade-up" style={{ animationDelay: '.12s' }}>
            <div className="pointer-events-none absolute -inset-5 rounded-[2.5rem] bg-gradient-to-br from-[#a78bfa]/40 via-white/20 to-[#f87171]/25 blur-2xl" />
            <div className="relative mx-auto max-w-[380px] overflow-hidden rounded-[1.75rem] bg-white shadow-[0_28px_70px_-20px_rgba(124,58,237,.4)] ring-1 ring-[#7c3aed]/20">
              <LazyImg src={IMG(1)} alt="Crème ciblée contre les lipomes" aspect="4/5" priority />
            </div>
            <div className="absolute -left-2 top-8 rotate-[-6deg] rounded-lg bg-[#1e1145] px-3 py-2 shadow-xl">
              <p className="text-[8px] font-black uppercase tracking-wider text-[#ddd6fe]">Premiers signes</p>
              <p className="cal-shine text-[15px] font-black">2-4 sem.</p>
            </div>
            <div className="absolute -right-2 bottom-10 rotate-[5deg] rounded-lg bg-white px-3 py-2 shadow-xl ring-1 ring-[#7c3aed]/20">
              <p className="text-[8px] font-black uppercase tracking-wider text-[#6d28d9]">4,9/5</p>
          ...
          <div className="mt-7 cal-fade-up" style={{ animationDelay: '.16s' }}>
            <div className="flex items-baseline justify-center gap-2 sm:gap-3">
              <span className="bg-gradient-to-r from-[#6d28d9] to-[#7c3aed] bg-clip-text text-4xl font-black text-transparent sm:text-5xl">{fmtTotal(1)}</span>
      ...
      {/* Stats */}
      <section className="border-y border-[#7c3aed]/15 bg-white">
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-3 px-4 py-5 sm:grid-cols-4 sm:py-6">
          {[
            ...
            <div key={i} className="text-center">
              <p className="bg-gradient-to-r from-[#6d28d9] to-[#7c3aed] bg-clip-text text-[22px] font-black text-transparent sm:text-[28px]">{s.n}</p>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#1e1145] via-[#1e1145]/75 to-[#1e1145]/30" />
        <div className="relative z-10 mx-auto max-w-xl px-4 py-16 text-center text-white sm:py-20">
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-[#ddd6fe]">La promesse</p>
      ...
      <section className="bg-gradient-to-b from-[#f5f3ff] to-white py-12">
        <div className="mx-auto max-w-xl px-4">
          <div className="text-center">
            <span className="rounded-full bg-[#7c3aed]/12 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-[#6d28d9]">Avis vérifiés</span>
            <h2 className="mt-3 text-[24px] font-black text-[#1e1145] sm:text-3xl">Elles l’ont fait <Hot>avant vous</Hot>.</h2>
          </div>
          <div className="relative mt-6 min-h-[180px] overflow-hidden rounded-2xl bg-white p-5 shadow-lg ring-1 ring-[#7c3aed]/12">
            {reviews.map((r, i) => (
              <div key={r.name} className={`absolute inset-x-5 top-5 transition-all duration-500 ${i === carouselIdx ? 'z-10 opacity-100' : 'pointer-events-none z-0 opacity-0'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[12px] font-black text-[#6d28d9]">{r.name}</p>
                ...
                <p className="mt-3 text-[15px] font-medium leading-relaxed text-[#1e1145]">« {r.txt} »</p>
            ...
            <div className="mt-[150px] flex justify-center gap-1.5">
              {reviews.map((_, i) => (
                <button key={i} type="button" aria-label={`Avis ${i + 1}`} onClick={() => setCarouselIdx(i)}
                  className={`h-1.5 rounded-full transition-all ${i === carouselIdx ? 'w-7 bg-[#7c3aed]' : 'w-1.5 bg-[#ddd6fe]'}`} />
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-md px-4">
          <div className="overflow-hidden rounded-2xl shadow-xl">
            <div className="flex items-center gap-3 bg-[#075e54] px-4 py-3 text-white">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#7c3aed] to-[#dc2626] text-sm font-black text-white">GS</div>
              <div><p className="text-[13px] font-black">Support GS · Anti-lipome</p><p className="text-[10px] text-[#ddd6fe]">● en ligne</p></div>
            </div>
            <div className="space-y-2 bg-[#ece5dd] px-3 py-4">
              <div className="max-w-[82%] rounded-2xl rounded-tl-sm bg-white px-3 py-2 shadow-sm">
                <p className="text-[11px] font-black text-[#6d28d9]">Aminata K.</p>
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-2xl px-4">
          <div className="text-center">
            <span className="rounded-full bg-[#7c3aed]/12 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-[#6d28d9]">Votre pack</span>
            <h2 className="mt-3 text-[24px] font-black text-[#1e1145] sm:text-3xl">Plus de tubes, <Hot red>moins cher</Hot>.</h2>
          </div>
            ...
            {[
              ...
              <button key={b.v} type="button" onClick={() => openModal(b.v)}
                className={`group relative overflow-hidden rounded-2xl border-2 p-4 text-left transition hover:scale-[1.02] hover:shadow-lg ${
                  b.hot ? 'border-[#dc2626] bg-gradient-to-br from-[#fef2f2] to-white ring-2 ring-[#f87171]/40' : 'border-[#7c3aed]/20 bg-[#f8fafc]'
                }`}>
                ...
                <p className="text-[9px] font-black uppercase tracking-wider text-[#6d28d9]">{perTube(b.v)} F / tube</p>
                <p className="mt-1 text-lg font-black text-[#1e1145]">{b.n}</p>
                <p className="mt-1.5 text-2xl font-black text-[#7c3aed]">{fmtF(b.p)} F</p>
          ...
          <div className="mt-5 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-[#7c3aed]/30 bg-[#f5f3ff] p-4 text-center sm:flex-row sm:text-left">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#7c3aed] text-2xl text-white">🎁</div>
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase text-[#6d28d9]">Bonus pack 3</p>
              <p className="text-[13px] font-black text-[#1e1145]">+1 mini-tube offert</p>
            </div>
            <button type="button" onClick={() => openModal(3)} className="shrink-0 rounded-full bg-[#7c3aed] px-4 py-2 text-[11px] font-black text-white">J’en profite</button>
          </div>
        </div>
      </section>
      ...
      {/* Garantie */}
      <section className="bg-[#f5f3ff] py-10">
        <div className="mx-auto max-w-2xl px-4">
          <div className="rounded-2xl bg-gradient-to-br from-[#6d28d9] via-[#7c3aed] to-[#c026d3] p-6 text-white shadow-xl shadow-violet-500/25">
            <p className="text-[10px] font-black uppercase tracking-wider text-[#ede9fe]">Zéro risque</p>
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-2xl px-4">
          <h2 className="text-center text-[24px] font-black text-[#1e1145]">Questions <Hot>fréquentes</Hot></h2>
          <div className="mt-6 space-y-2">
            {[
              ...
              <details key={i} className="group overflow-hidden rounded-xl bg-[#f8fafc] ring-1 ring-[#7c3aed]/12 open:ring-[#7c3aed]/30">
                <summary className="flex cursor-pointer items-center justify-between px-4 py-3.5 text-[14px] font-black text-[#1e1145]">
                  {f.q}
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[#1e1145]/94 via-[#0f2a52]/90 to-[#1e1145]/92" />
        <div className="relative z-10 mx-auto max-w-xl px-4 py-16 text-center text-white sm:py-20">
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-[#ddd6fe]">Dernière chance</p>
          ...
          <div className="mx-auto mt-6 max-w-sm">
            ...
            <p className="mt-2 text-[11px] font-semibold text-[#ddd6fe]/80">Paiement uniquement à la réception</p>
          </div>
        </div>
      </section>

      <footer className="bg-[#1e1145] py-6 text-center text-[10px] font-medium text-[#64748b]">
        © {new Date().getFullYear()} · Crème anti-lipome · Usage externe · Résultats variables selon les individus
      </footer>

      {/* Sticky mobile CTA */}
      <div className={`fixed inset-x-0 bottom-0 z-40 border-t border-[#8b5cf6]/25 bg-[#1e1145]/95 px-3 py-2 backdrop-blur-md transition-all sm:hidden ${modal ? 'translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}
        style={{ paddingBottom: 'calc(0.5rem + env(safe-area-inset-bottom, 0px))' }}>
        <div className="flex items-center gap-3">
          <div className="flex-1">
            <p className="text-[9px] font-black uppercase text-[#ddd6fe]">{pad(countdown.h)}:{pad(countdown.m)}:{pad(countdown.s)}</p>
            <p className="text-[12px] font-black text-white">{fmtTotal(1)} F</p>
          </div>
          <button type="button" onClick={() => openModal(1)}
            className="cal-shake inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[#6d28d9] via-[#7c3aed] to-[#c026d3] px-5 py-2.5 text-[12px] font-black text-white shadow-lg shadow-violet-500/40">
            Commander <Arrow />
          </button>
        </div>
      </div>
