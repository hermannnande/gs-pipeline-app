/**
 * Page merci — Répulsif Ultrasons Anti-Nuisibles (slug repulsif-ultrasons).
...
export default function RepulsifUltrasonsThankYou() {
  ...
  useEffect(() => { document.title = 'Merci — Répulsif Ultrasons Anti-Nuisibles'; }, []);

  useEffect(() => {
    if (fired.current) return; fired.current = true;
    const sk = ref ? `ru_purchase_${ref}` : '';
    ...
    if (META_PIXEL_ID) {
      const init = () => {
        ...
        window.fbq('track', 'Purchase', { content_name: 'Répulsif Ultrasons Anti-Nuisibles', content_ids: [PRODUCT_CODE], value: orderTotal(PRICES, qty), currency: 'XOF', num_items: qty });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#EFF6FF] via-[#F0F9FF] to-[#FFF7ED] px-4 py-10">
      <div className="w-full max-w-md overflow-hidden rounded-[28px] border border-[#93C5FD]/40 bg-white shadow-2xl">
        <div className="bg-gradient-to-br from-[#1E3A8A] via-[#2563EB] to-[#06B6D4] px-6 py-8 text-center text-white">
          ...
          <p className="mt-1.5 text-[13px] text-white/85">Votre Répulsif Ultrasons Anti-Nuisibles est bien enregistré.</p>
        </div>
        <div className="space-y-4 p-5">
          {ref && (
            <div className="rounded-xl bg-[#EFF6FF] p-3 text-center ring-1 ring-[#93C5FD]/40">
              ...
              <p className="font-mono text-sm font-bold text-[#0A1F44]">{ref.slice(0, 12).toUpperCase()}</p>
            </div>
          )}
          <div className="space-y-2.5 rounded-2xl bg-[#F0F9FF] p-4 text-[12.5px] text-[#1E3A8A]">
          ...
          <p className="text-center text-[12px] text-neutral-500">
            Gardez votre téléphone à portée de main : sans confirmation, le boîtier ne peut pas être expédié.
          </p>
          <Link to="/repulsif-ultrasons" className="block rounded-full bg-gradient-to-r from-[#FB923C] via-[#F97316] to-[#EA580C] py-3.5 text-center text-[12px] font-black uppercase tracking-[0.12em] text-white shadow-lg transition hover:brightness-110">
            Retour au produit
          </Link>
        </div>
      </div>
