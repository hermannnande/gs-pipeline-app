/**
 * Page merci — Oxymètre de Pouls Digital, variante TIKTOK (slug oxymetre-pouls-tk).
...
const PRODUCT_CODE = 'OXYMETRE_POULS_TK'; // Produit dedie (meme mapping que la landing -tk)
...
export default function OxymetresPoulsTkThankYou() {
  ...
  useEffect(() => {
    ...
    if (TIKTOK_PIXEL_ID && window.ttq) {
      window.ttq.track('CompletePayment', { content_name: 'Oxymètre de Pouls Digital', content_id: PRODUCT_CODE, content_type: 'product', value: orderTotal(PRICES, qty), currency: 'XOF', quantity: qty });
      if (sk) sessionStorage.setItem(sk, '1');
    }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#EEF2FF] via-[#E0E7FF] to-[#F5F3FF] px-4 py-10">
      <div className="w-full max-w-md overflow-hidden rounded-[28px] border border-[#A78BFA]/40 bg-white shadow-2xl">
        <div className="bg-gradient-to-br from-[#4F46E5] via-[#7C3AED] to-[#06B6D4] px-6 py-8 text-center text-white">
        ...
        <div className="space-y-4 p-5">
          {ref && (
            <div className="rounded-xl bg-[#EEF2FF] p-3 text-center ring-1 ring-[#A78BFA]/40">
          ...
          <Link to="/oxymetre-pouls-tk" className="block rounded-full bg-gradient-to-r from-[#4F46E5] via-[#7C3AED] to-[#EF4444] py-3.5 text-center text-[12px] font-black uppercase tracking-[0.12em] text-white shadow-lg transition hover:brightness-110">
            Retour au produit
          </Link>
        </div>
      </div>
