  ...
  return (
    <div ref={ref} className={`overflow-hidden ${className}`} style={aspect ? { aspectRatio: aspect } : undefined}>
      {visible ? (
        ...
        <div className="h-full min-h-[240px] w-full animate-pulse bg-[#d1fae5]/80" />
      )}
    </div>
  );
}
...
function LazyVideo({ src, poster }: { src: string; poster?: string }) {
  const { ref, visible } = useOnScreen();
  return (
    <div ref={ref} className="relative w-full overflow-hidden bg-[#03211a]" style={{ aspectRatio: '9/16' }}>
      {visible ? (
        <video src={src} poster={poster} autoPlay loop muted playsInline preload="none" className="h-full w-full object-cover" />
      ) : (
        <div className="flex h-full w-full items-center justify-center bg-[#06342a]/60">
          {poster ? <img src={poster} alt="" className="absolute inset-0 h-full w-full object-cover opacity-40" /> : null}
          <div className="relative h-10 w-10 animate-spin rounded-full border-2 border-white/20 border-t-[#34d399]" />
        </div>
      )}
      <span className="pointer-events-none absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#03211a]/90 to-transparent" />
...
const Star = () => (
  <svg className="h-3.5 w-3.5 text-[#fbbf24]" fill="currentColor" viewBox="0 0 20 20">
...
function Hot({ children, gold }: { children: ReactNode; gold?: boolean }) {
  return (
    <span className={gold
      ? 'bg-gradient-to-r from-[#f59e0b] via-[#fbbf24] to-[#fde68a] bg-clip-text font-black text-transparent'
      : 'bg-gradient-to-r from-[#047857] via-[#10b981] to-[#34d399] bg-clip-text font-black text-transparent'
    }>
      {children}
    </span>
  );
...
/** CTA principal : dégradé émeraude→or, reflet balayant + tremblement périodique. */
function FluidCTA({ onClick, children }: { onClick: () => void; children: ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="cal-cta cal-shake group relative inline-flex w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-gradient-to-r from-[#047857] via-[#10b981] to-[#f59e0b] px-6 py-4 text-[15px] font-black text-white shadow-[0_16px_44px_-10px_rgba(16,185,129,.6)] ring-2 ring-white/30 transition hover:scale-[1.015] hover:shadow-[0_20px_50px_-8px_rgba(245,158,11,.5)] active:scale-[0.99] sm:text-[16px]"
    >
...
function Marquee({ items }: { items: string[] }) {
  return (
    <div className="overflow-hidden border-y border-[#10b981]/25 bg-gradient-to-r from-[#03211a] via-[#065f46] to-[#03211a] py-2.5">
      <div className="cal-marquee flex w-[200%] items-center gap-8 text-[10px] font-black uppercase tracking-[0.2em] text-[#d1fae5] sm:text-[11px]">
        {[0, 1].map((k) => (
          <div key={k} className="flex shrink-0 items-center gap-8">
            {items.map((t, i) => (
              <span key={`${k}-${i}`} className="inline-flex items-center gap-2">
                {t}<span className="text-[#fbbf24]">✦</span>
              </span>
            ))}
          </div>
        ))}
...
function Fiche({ kicker, hook, cta, qty, onOrder, media, variant = 'light', shape = 'card' }: FicheProps) {
  const bg =
    variant === 'dark'
      ? 'bg-gradient-to-b from-[#03211a] via-[#065f46] to-[#03211a] text-white'
      : variant === 'warm'
        ? 'bg-gradient-to-b from-[#ecfdf5] via-white to-[#fffbeb] text-[#03211a]'
        : 'bg-gradient-to-b from-[#f8fafc] via-white to-[#ecfdf5] text-[#03211a]';

  const mediaWrap =
    shape === 'tilt'
      ? 'relative mx-auto max-w-[440px] -rotate-[1.2deg] overflow-hidden rounded-[1.75rem] shadow-[0_24px_60px_-18px_rgba(5,150,105,.35)] ring-1 ring-[#10b981]/30'
      : 'relative mx-auto max-w-[440px] overflow-hidden rounded-[1.75rem] shadow-[0_24px_60px_-18px_rgba(5,150,105,.3)] ring-1 ring-[#10b981]/25';

  return (
    <section className={`relative overflow-hidden py-11 sm:py-14 ${bg}`}>
      <div className="pointer-events-none absolute -left-20 top-8 h-48 w-48 rounded-full bg-[#34d399]/25 blur-3xl cal-float" />
      <div className="relative mx-auto max-w-xl px-4 text-center">
        {kicker && (
          <p className={`mb-2.5 text-[10px] font-black uppercase tracking-[0.28em] ${variant === 'dark' ? 'text-[#a7f3d0]' : 'text-[#059669]'}`}>
            {kicker}
          </p>
        )}
        ...
        <div className="mt-6">
          ...
          <p className={`mt-2 text-[11px] font-semibold ${variant === 'dark' ? 'text-[#a7f3d0]/80' : 'text-[#64748b]'}`}>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]" style={{ fontFamily: 'system-ui,-apple-system,Segoe UI,Roboto,sans-serif' }}>
      <style>{`
        ...
        .cal-gold { background: linear-gradient(90deg,#d97706,#fbbf24,#fef3c7,#fbbf24,#d97706); background-size:200% auto; background-clip:text; -webkit-background-clip:text; color:transparent; -webkit-text-fill-color:transparent; animation:cal-shimmer 3.2s linear infinite }
        ...
        /* CTA qui tremble : secousse courte toutes les ~4 s.
           ...
           qui repeindrait chaque frame sur les ~10 CTA de la page. */
        @keyframes cal-shake {
        ...
        @media (prefers-reduced-motion: reduce) {
          .cal-shake, .cal-sheen, .cal-float, .cal-marquee, .cal-gold { animation: none !important }
        }
      `}</style>

      {/* Sticky header */}
      <div className="sticky top-0 z-50 border-b border-[#10b981]/30 bg-gradient-to-r from-[#03211a]/98 via-[#065f46]/95 to-[#03211a]/98 shadow-lg backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-2 px-3 py-2 sm:gap-3">
          <span className="relative flex h-2 w-2 text-[#6ee7b7] cal-pulse-dot">
            <span className="relative inline-flex h-2 w-2 rounded-full bg-[#6ee7b7]" />
          </span>
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#d1fae5]">
            <span className="cal-gold">Offre limitée</span> · fin à minuit
          </span>
          <div className="flex items-center gap-1">
            {[countdown.h, countdown.m, countdown.s].map((v, i) => (
              <span key={i} className="flex items-center gap-1">
                {i > 0 && <span className="text-[#10b981]">:</span>}
                <span className="inline-flex h-7 min-w-[30px] items-center justify-center rounded-md bg-[#10b981]/25 px-1 font-mono text-[12px] font-black tabular-nums text-white ring-1 ring-[#6ee7b7]/30">
                  {pad(v)}
                </span>
              </span>
            ))}
          </div>
          <span className="rounded-full bg-[#fbbf24] px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider text-[#03211a]">
            {stock} restants
          </span>
        </div>
        <div className="h-[2px] w-full bg-[#06342a]">
          <div className="h-full bg-gradient-to-r from-[#fbbf24] via-[#34d399] to-[#10b981] transition-all duration-700" style={{ width: `${stockPct}%` }} />
        </div>
      </div>

      <Marquee items={['STOP AUX LIPOMES', 'FORMULE BOTANIQUE', 'RÉSULTATS VISIBLES', 'PAIEMENT À LA LIVRAISON', 'EXPRESS 24H']} />

      {/* Hero */}
      <section className="relative overflow-hidden bg-gradient-to-b from-[#ecfdf5] via-white to-[#fffbeb] py-10 sm:py-14">
        <div className="pointer-events-none absolute -left-16 -top-12 h-64 w-64 rounded-full bg-[#34d399]/35 blur-3xl cal-float" />
        <div className="pointer-events-none absolute -right-12 top-20 h-48 w-48 rounded-full bg-[#fbbf24]/25 blur-3xl cal-float" style={{ animationDelay: '2s' }} />
        <div className="relative mx-auto max-w-xl px-4 text-center">
          <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#059669]/25 bg-white/90 px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.24em] text-[#047857] cal-fade-up">
            N°1 contre les lipomes en Côte d’Ivoire
          </p>

          <h1 className="mt-3 text-balance text-[34px] font-black leading-[1.04] tracking-tight text-[#03211a] sm:text-[46px] cal-fade-up" style={{ animationDelay: '.04s' }}>
            <Hot>Stop</Hot> aux <Hot gold>bosses</Hot><br />sous la peau.
          </h1>
          ...
          <div className="relative mt-7 cal-fade-up" style={{ animationDelay: '.12s' }}>
            <div className="pointer-events-none absolute -inset-5 rounded-[2.5rem] bg-gradient-to-br from-[#34d399]/40 via-white/20 to-[#fbbf24]/30 blur-2xl" />
            <div className="relative mx-auto max-w-[380px] overflow-hidden rounded-[1.75rem] bg-white shadow-[0_28px_70px_-20px_rgba(5,150,105,.4)] ring-1 ring-[#059669]/20">
              <LazyImg src={IMG(1)} alt="Crème anti-lipome — formule botanique verte" aspect="4/5" priority />
            </div>
            <div className="absolute -left-2 top-8 rotate-[-6deg] rounded-lg bg-[#03211a] px-3 py-2 shadow-xl">
              <p className="text-[8px] font-black uppercase tracking-wider text-[#a7f3d0]">Premiers signes</p>
              <p className="cal-gold text-[15px] font-black">2-4 sem.</p>
            </div>
            <div className="absolute -right-2 bottom-10 rotate-[5deg] rounded-lg bg-white px-3 py-2 shadow-xl ring-1 ring-[#059669]/20">
              <p className="text-[8px] font-black uppercase tracking-wider text-[#047857]">4,9/5</p>
          ...
          <div className="mt-7 cal-fade-up" style={{ animationDelay: '.16s' }}>
            <div className="flex items-baseline justify-center gap-2 sm:gap-3">
              <span className="bg-gradient-to-r from-[#047857] to-[#10b981] bg-clip-text text-4xl font-black text-transparent sm:text-5xl">{fmtTotal(1)}</span>
              ...
              <span className="rounded-md bg-[#03211a] px-2 py-0.5 text-[9px] font-black uppercase text-[#fbbf24]">-{DISCOUNT}%</span>
            </div>
      ...
      {/* Stats */}
      <section className="border-y border-[#059669]/15 bg-white">
        <div className="mx-auto grid max-w-4xl grid-cols-2 gap-3 px-4 py-5 sm:grid-cols-4 sm:py-6">
          {[
            ...
            <div key={i} className="text-center">
              <p className="bg-gradient-to-r from-[#047857] to-[#10b981] bg-clip-text text-[22px] font-black text-transparent sm:text-[28px]">{s.n}</p>
      ...
      <Fiche
        kicker="Regardez"
        hook={<>Elle <Hot gold>fond</Hot> là où les autres <Hot>échouent</Hot>.</>}
      ...
      <Fiche
        kicker="La formule"
        hook={<>Des actifs botaniques <Hot>concentrés</Hot>. Zéro bistouri.</>}
        cta="Oui, je veux cette crème"
        qty={1}
        onOrder={openModal}
        variant="light"
        media={<LazyImg src={IMG(2)} alt="Formule botanique anti-lipome" aspect="1/1" />}
      />

      {/* Bannière pleine largeur */}
      <section className="relative overflow-hidden">
        <LazyImg src={IMG(3)} alt="Avant après crème anti-lipome" aspect="16/10" className="absolute inset-0 h-full w-full" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#03211a] via-[#03211a]/75 to-[#03211a]/30" />
        <div className="relative z-10 mx-auto max-w-xl px-4 py-16 text-center text-white sm:py-20">
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-[#a7f3d0]">La promesse</p>
          <h2 className="mt-3 text-balance text-[28px] font-black leading-tight sm:text-[36px]">
            Plus jamais honte de vos <span className="cal-gold">bras</span>.
          </h2>
      ...
      <Fiche
        ...
        hook={<>Bras, dos, nuque, jambes — <Hot gold>partout</Hot>.</>}
      ...
      <Fiche
        kicker="Le geste"
        hook={<><Hot gold>30 secondes</Hot>, matin et soir. C’est tout.</>}
      ...
      {/* Témoignages */}
      <section className="bg-gradient-to-b from-[#ecfdf5] to-white py-12">
        <div className="mx-auto max-w-xl px-4">
          <div className="text-center">
            <span className="rounded-full bg-[#059669]/12 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-[#047857]">Avis vérifiés</span>
            <h2 className="mt-3 text-[24px] font-black text-[#03211a] sm:text-3xl">Elles l’ont fait <Hot>avant vous</Hot>.</h2>
          </div>
          <div className="relative mt-6 min-h-[180px] overflow-hidden rounded-2xl bg-white p-5 shadow-lg ring-1 ring-[#059669]/12">
            {reviews.map((r, i) => (
              <div key={r.name} className={`absolute inset-x-5 top-5 transition-all duration-500 ${i === carouselIdx ? 'z-10 opacity-100' : 'pointer-events-none z-0 opacity-0'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[12px] font-black text-[#047857]">{r.name}</p>
                ...
                <p className="mt-3 text-[15px] font-medium leading-relaxed text-[#03211a]">« {r.txt} »</p>
            ...
            <div className="mt-[150px] flex justify-center gap-1.5">
              {reviews.map((_, i) => (
                <button key={i} type="button" aria-label={`Avis ${i + 1}`} onClick={() => setCarouselIdx(i)}
                  className={`h-1.5 rounded-full transition-all ${i === carouselIdx ? 'w-7 bg-[#059669]' : 'w-1.5 bg-[#a7f3d0]'}`} />
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-md px-4">
          <div className="overflow-hidden rounded-2xl shadow-xl">
            <div className="flex items-center gap-3 bg-[#075e54] px-4 py-3 text-white">
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#10b981] to-[#fbbf24] text-sm font-black text-white">GS</div>
              <div><p className="text-[13px] font-black">Support GS · Anti-lipome</p><p className="text-[10px] text-[#a7f3d0]">● en ligne</p></div>
            </div>
            <div className="space-y-2 bg-[#ece5dd] px-3 py-4">
              <div className="max-w-[82%] rounded-2xl rounded-tl-sm bg-white px-3 py-2 shadow-sm">
                <p className="text-[11px] font-black text-[#047857]">Aminata K.</p>
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-2xl px-4">
          <div className="text-center">
            <span className="rounded-full bg-[#059669]/12 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-[#047857]">Votre pack</span>
            <h2 className="mt-3 text-[24px] font-black text-[#03211a] sm:text-3xl">Plus de tubes, <Hot gold>moins cher</Hot>.</h2>
          </div>
            ...
            {[
              ...
              <button key={b.v} type="button" onClick={() => openModal(b.v)}
                className={`group relative overflow-hidden rounded-2xl border-2 p-4 text-left transition hover:scale-[1.02] hover:shadow-lg ${
                  b.hot ? 'border-[#f59e0b] bg-gradient-to-br from-[#fffbeb] to-white ring-2 ring-[#fbbf24]/40' : 'border-[#059669]/20 bg-[#f8fafc]'
                }`}>
                {b.hot && <span className="absolute -top-0.5 right-3 rounded-b-md bg-[#f59e0b] px-2 py-0.5 text-[8px] font-black uppercase text-[#03211a]">★ Populaire</span>}
                <p className="text-[9px] font-black uppercase tracking-wider text-[#047857]">{perTube(b.v)} F / tube</p>
                <p className="mt-1 text-lg font-black text-[#03211a]">{b.n}</p>
                <p className="mt-1.5 text-2xl font-black text-[#059669]">{fmtF(b.p)} F</p>
                <p className="text-[11px] text-[#94a3b8] line-through">{fmtF(OLD_UNIT * b.v)} F</p>
                {b.save && <p className="mt-1.5 inline-flex rounded-full bg-[#059669] px-2 py-0.5 text-[8px] font-black uppercase text-white">{b.save}</p>}
              </button>
            ))}
          </div>
          <div className="mt-5 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-[#059669]/30 bg-[#ecfdf5] p-4 text-center sm:flex-row sm:text-left">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#059669] text-2xl text-white">🎁</div>
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase text-[#047857]">Bonus pack 3</p>
              <p className="text-[13px] font-black text-[#03211a]">+1 mini-tube offert</p>
            </div>
            <button type="button" onClick={() => openModal(3)} className="shrink-0 rounded-full bg-[#059669] px-4 py-2 text-[11px] font-black text-white">J’en profite</button>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#ecfdf5] py-10">
        <div className="mx-auto max-w-2xl px-4">
          <div className="rounded-2xl bg-gradient-to-br from-[#047857] via-[#10b981] to-[#f59e0b] p-6 text-white shadow-xl shadow-emerald-500/25">
            <p className="text-[10px] font-black uppercase tracking-wider text-[#d1fae5]">Zéro risque</p>
            <h3 className="mt-2 text-[22px] font-black sm:text-2xl">Vous payez <span className="cal-gold">à la livraison</span>.</h3>
            <ul className="mt-4 space-y-2 text-[13px] font-semibold">
              {['Testée dermatologiquement', 'Livraison 24h Abidjan · 48h régions', 'Cash au livreur, après vérification', 'Conseiller WhatsApp dispo'].map((x, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#fbbf24] text-[#03211a]">
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-2xl px-4">
          <h2 className="text-center text-[24px] font-black text-[#03211a]">Questions <Hot>fréquentes</Hot></h2>
          <div className="mt-6 space-y-2">
            {[
              { q: 'En combien de temps ?', a: 'Premiers signes dès 2 semaines, résultat net entre 4 et 8 semaines.' },
              { q: 'Sur quelles zones ?', a: 'Tous les lipomes superficiels : bras, dos, nuque, mains, jambes. Usage externe.' },
              ...
              <details key={i} className="group overflow-hidden rounded-xl bg-[#f8fafc] ring-1 ring-[#059669]/12 open:ring-[#059669]/30">
                <summary className="flex cursor-pointer items-center justify-between px-4 py-3.5 text-[14px] font-black text-[#03211a]">
                  {f.q}
                  <svg className="h-4 w-4 shrink-0 text-[#059669] transition-transform group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[#03211a]/94 via-[#06342a]/90 to-[#03211a]/92" />
        <div className="relative z-10 mx-auto max-w-xl px-4 py-16 text-center text-white sm:py-20">
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-[#a7f3d0]">Dernière chance</p>
          <h2 className="mt-3 text-balance text-3xl font-black leading-tight sm:text-4xl">
            Demain, elle sera <span className="cal-gold">encore là</span>.
          </h2>
          <div className="mt-5 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 ring-1 ring-[#6ee7b7]/30">
          ...
          <div className="mx-auto mt-6 max-w-sm">
            ...
            <p className="mt-2 text-[11px] font-semibold text-[#a7f3d0]/80">Paiement uniquement à la réception</p>
          </div>
        </div>
      </section>

      <footer className="bg-[#03211a] py-6 text-center text-[10px] font-medium text-[#64748b]">
        © {new Date().getFullYear()} · Crème anti-lipome · Usage externe · Résultats variables selon les individus
      </footer>

      {/* Sticky mobile CTA */}
      <div className={`fixed inset-x-0 bottom-0 z-40 border-t border-[#10b981]/25 bg-[#03211a]/95 px-3 py-2 backdrop-blur-md transition-all sm:hidden ${modal ? 'translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}
        style={{ paddingBottom: 'calc(0.5rem + env(safe-area-inset-bottom, 0px))' }}>
        <div className="flex items-center gap-3">
          <div className="flex-1">
            <p className="text-[9px] font-black uppercase text-[#a7f3d0]">{pad(countdown.h)}:{pad(countdown.m)}:{pad(countdown.s)}</p>
            <p className="text-[12px] font-black text-white">{fmtTotal(1)} F</p>
          </div>
          <button type="button" onClick={() => openModal(1)}
            className="cal-shake inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[#047857] via-[#10b981] to-[#f59e0b] px-5 py-2.5 text-[12px] font-black text-white shadow-lg shadow-emerald-500/40">
            Commander <Arrow />
          </button>
        </div>
      </div>
