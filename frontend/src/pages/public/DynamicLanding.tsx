    ...
    <div className="min-h-screen bg-white text-neutral-900" style={{ fontFamily: "'Inter',system-ui,-apple-system,sans-serif" }}>
            ...
            <div className="hidden sm:block">
              <GlowBtn onClick={open} theme={themeKey}>
                <span className="relative z-10 flex items-center gap-2">
                  ...
                  Commander maintenant Ã¢â‚¬â€ {fmt(prices[1] || 0)}
                </span>
              </GlowBtn>
            </div>
          </div>
        ...
        <div className="relative overflow-hidden py-6 sm:py-8">
          <div className="relative mx-auto max-w-3xl px-4">
            ...
            <div className="rounded-b-2xl border border-t-0 border-neutral-100 bg-gradient-to-br from-neutral-900 via-neutral-800 to-neutral-900 px-5 py-6 text-center shadow-xl sm:px-8 sm:py-8">
              ...
              <GlowBtn onClick={open} theme={themeKey}>
                <span className="relative z-10 flex items-center gap-2">Commander maintenant Ã¢â‚¬â€ {fmt(prices[1] || 0)}</span>
              </GlowBtn>
            </div>
          </div>
        </div>
      ...
      <LazySection className="relative overflow-hidden py-14 sm:py-20">
        ...
        <div className="relative mx-auto max-w-lg px-4 text-center">
          ...
          <GlowBtn onClick={open} theme={themeKey}>
            <span className="relative z-10">Commander ici Ã¢â‚¬â€ {fmt(prices[1] || 0)}</span>
          </GlowBtn>
          <p className="mt-3 text-[11px] text-neutral-500">Aucun compte requis Ã‚Â· Paiement a la livraison</p>
        </div>
      </LazySection>
        ...
        <div className="mx-auto max-w-4xl px-4">
          <div className="flex flex-wrap justify-center gap-6 text-center">
            {(cfg.sections?.trustBadges || [
              { ico: 'Ã°Å¸Å¡Å¡', t: 'Livraison rapide', d: '24h Abidjan' },
              { ico: 'Ã°Å¸â€™Â°', t: 'Paiement livraison', d: 'Aucun risque' },
              { ico: 'Ã°Å¸â€œÅ¾', t: 'Support client', d: '7j/7' },
            ]).map((f, i) => (
          ...
          <p className="mt-6 text-center text-[10px] text-neutral-300">Ã‚Â© 2026 Ã‚Â· Cote d'Ivoire</p>
        </div>
      </footer>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-neutral-200/80 bg-white/95 backdrop-blur-lg">
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-2.5 sm:py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-neutral-400">{fmt(prices[1] || 0)} Ã‚Â· Paiement a la livraison</p>
          </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center sm:p-4" onClick={e => { if (e.target === e.currentTarget) setModal(false); }}>
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm"/>
          <div className="relative max-h-[90vh] w-full overflow-y-auto rounded-t-2xl bg-white shadow-2xl sm:max-h-[92vh] sm:max-w-[400px] sm:rounded-2xl">
            ...
            <form onSubmit={submit} className="space-y-2.5 p-3 pb-4 sm:space-y-3 sm:p-4 sm:pb-5">
              ...
              <div className="flex items-center justify-between rounded-xl bg-neutral-900 px-3 py-2.5 text-white">
                ...
                <span className="text-[15px] font-black">{fmt(prices[qty] || prices[1] || 0)}</span>
              </div>
              {formErr && <p className="rounded-lg bg-red-50 px-3 py-2 text-[11px] font-semibold text-red-600">{formErr}</p>}
              <button type="submit" disabled={sending} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 text-[13px] font-extrabold text-white shadow-[0_12px_30px_rgba(16,185,129,.3)] hover:bg-emerald-500 active:scale-[.98] disabled:cursor-wait disabled:opacity-70">
              ...
              <p className="text-center text-[9px] text-neutral-400">Nous vous appelons pour confirmer.</p>
            </form>
          </div>
        </div>
      )}
