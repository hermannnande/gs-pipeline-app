/**
 * Page merci — Bandes Buccales pour le Sommeil, variante TIKTOK (slug bandes-buccales-sommeil-tk).
 * Theme NUIT coherent avec la landing. Pixel Meta desactive — Pixel TikTok :
 * renseigner TIKTOK_PIXEL_ID (event CompletePayment, deduplique par sessionStorage).
 */
...
export default function BandesBuccalesSommeilTkThankYou() {
  ...
  useEffect(() => {
    ...
    if (TIKTOK_PIXEL_ID && window.ttq) {
      window.ttq.track('CompletePayment', { content_name: 'Bandes Buccales pour le Sommeil (paquet de 30)', content_id: PRODUCT_CODE, content_type: 'product', value: orderTotal(PRICES, qty), currency: 'XOF', quantity: qty });
      if (sk) sessionStorage.setItem(sk, '1');
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-[#0A0A0F] via-[#111118] to-[#0A0A0F] px-4 py-10">
      <div className="w-full max-w-md overflow-hidden rounded-[28px] border border-white/15 bg-white/5 shadow-2xl backdrop-blur-md">
        ...
        <div className="space-y-4 p-5">
          ...
          <Link to="/bandes-buccales-sommeil-tk" className="block rounded-full bg-gradient-to-r from-[#F59E0B] via-[#FCD34D] to-[#F59E0B] py-3.5 text-center text-[12px] font-black uppercase tracking-[0.12em] text-[#1A1207] shadow-lg transition hover:brightness-110">
            Retour au produit
          </Link>
        </div>
      </div>
