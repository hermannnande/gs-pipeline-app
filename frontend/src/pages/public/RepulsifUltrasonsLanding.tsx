 ...
 * Pattern : copie conforme de SupportTelephoneFlexibleLanding (validé client).
 */
...
const OrderModalRepulsifUltrasons = lazy(() => import('../../components/order/OrderModalRepulsifUltrasons'));

const API_URL = import.meta.env.VITE_API_URL || '/api';
const SLUG = 'repulsif-ultrasons';
const PRODUCT_CODE = 'REPULSIF_ULTRASONS';
const CONTENT_NAME = 'Répulsif Ultrasons Anti-Nuisibles';
...
const MEDIA = {
  ...
  marreNuisibles: M('n2.webp'),
  cuisineGif: M('n4.webp'),
  protegerMaison: M('n3.webp'),
  photoReelle: M('n5.webp'),
  priseSimple: M('n6.webp'),
  chaquePiece: M('n7.webp'),
};
  ...
  return (
    <div ref={ref} className="overflow-hidden" style={aspect ? { aspectRatio: aspect } : undefined}>
      {visible ? (
        ...
        <div className="h-full min-h-[260px] w-full animate-pulse bg-gradient-to-br from-[#93C5FD]/40 to-[#06B6D4]/20" />
      )}
    </div>
  );
}
...
function LazyVideo({ src, poster, badge }: { src: string; poster?: string; badge?: string }) {
  const { ref, visible } = useOnScreen();
  return (
    <div ref={ref} className="relative w-full overflow-hidden bg-[#0A1533]" style={{ aspectRatio: '9/16' }}>
      {visible ? (
...
function Star({ half }: { half?: boolean }) {
  return (
    <svg className="h-4 w-4 text-[#F59E0B]" fill={half ? 'url(#ruHalfStar)' : 'currentColor'} viewBox="0 0 20 20">
      {half && (
        <defs>
          <linearGradient id="ruHalfStar" x1="0" y1="0" x2="1" y2="0">
...
/* ------------------------------------------------------------------ */
/* CTA dégradé orange (rappel des visuels produit) + animations.       */
/* ------------------------------------------------------------------ */
function PestCTA({ onClick, children, big }: { onClick: () => void; children: ReactNode; big?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function StickyCTA({ visible, onClick }: { visible: boolean; onClick: () => void }) {
  return (
    ...
    >
      <div className="border-t border-white/25 bg-gradient-to-r from-[#1E3A8A]/95 via-[#2563EB]/95 to-[#06B6D4]/95 shadow-[0_-10px_34px_-10px_rgba(10,21,51,.45)] backdrop-blur-md">
...
function Marquee({ items, dark }: { items: string[]; dark?: boolean }) {
  return (
    <div className={`overflow-hidden py-2.5 ${dark
      ? 'border-y border-[#06B6D4]/30 bg-gradient-to-r from-[#0A1533] via-[#1E3A8A] to-[#0A1533] text-[#DBEAFE]'
      : 'border-y border-white/40 bg-gradient-to-r from-[#1E3A8A] via-[#2563EB] to-[#06B6D4] text-white'}`}>
      <div className="ru-marquee flex w-[200%] items-center gap-8 text-[10px] font-black uppercase tracking-[0.22em] sm:text-[11px]">
        {[0, 1].map((k) => (
  ...
  return (
    <section className={`px-4 py-8 sm:py-10 ${bg}`}>
      <div className="mx-auto max-w-[560px]">
        <div className={`overflow-hidden rounded-[28px] shadow-2xl ${dark ? 'ring-1 ring-white/25' : 'ring-1 ring-[#2563EB]/15'}`}>
          <div style={{ aspectRatio: ratio }} className="w-full [&>div]:h-full">{media}</div>
        </div>
        <div className="mt-5 text-center">
          <span className={`inline-flex rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[0.2em] ${dark ? 'bg-white/15 text-[#FDBA74] ring-1 ring-white/25' : 'bg-white/70 text-[#1D4ED8] ring-1 ring-[#2563EB]/30'}`}>
            {kicker}
          </span>
          <h2 className={`mt-3 text-[22px] font-black leading-tight sm:text-[26px] ${dark ? 'text-white' : 'text-[#0A1F44]'}`}>{title}</h2>
          <p className={`mx-auto mt-2 max-w-[420px] text-[13px] leading-relaxed sm:text-[14px] ${dark ? 'text-white/85' : 'text-[#1E3A8A]/75'}`}>{text}</p>
          <div className="mx-auto mt-5 max-w-sm"><PestCTA onClick={onCta}>{cta}</PestCTA></div>
        </div>
      </div>
    </section>
  );
...
const PURCHASE_NOTIFS = [
  { n: 'Koffi', q: 'Abobo', p: '1 boîtier' },
  { n: 'Aminata', q: 'Yopougon', p: '2 boîtiers' },
  { n: 'Yao', q: 'Cocody', p: '1 boîtier' },
  { n: 'Fatou', q: 'Marcory', p: '3 boîtiers' },
  { n: 'Ibrahim', q: 'Bouaké', p: '2 boîtiers' },
  { n: 'Awa', q: 'Angré', p: '1 boîtier' },
  { n: 'Moussa', q: 'Koumassi', p: '2 boîtiers' },
  { n: 'Adjoua', q: 'Treichville', p: '1 boîtier' },
  { n: 'Salimata', q: 'San-Pédro', p: '3 boîtiers' },
  { n: 'Nadia', q: 'Riviera', p: '2 boîtiers' },
];
...
function PurchaseNotifs() {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 rounded-2xl bg-white/95 p-3 pr-4 shadow-[0_16px_40px_-12px_rgba(10,21,51,.35)] ring-1 ring-[#93C5FD]/50 backdrop-blur-md">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#2563EB] to-[#06B6D4] text-[15px] font-black text-white">
          {it.n[0]}
        </span>
        <div className="min-w-0">
          <p className="text-[12px] font-bold leading-tight text-[#0A1F44]">
...
/* ------------------------------------------------------------------ */
const TESTIMONIALS = [
  { n: 'Koffi', v: 'Abobo', stars: 5, t: "Branché dans la chambre depuis 2 semaines : fini les moustiques qui bourdonnent à l'oreille. Les enfants dorment enfin tranquilles, sans spray ni spirale 🙏🏾" },
  { n: 'Aminata', v: 'Yopougon', stars: 5, t: "Les cafards ont déserté ma cuisine en quelques jours. J'étais sceptique sur les ultrasons, mais ça marche vraiment. Zéro odeur, zéro produit." },
  { n: 'Ibrahim', v: 'Bouaké', stars: 4, t: "Dans ma boutique, les souris ne touchent plus mes sacs de riz et mes cartons. Branché au mur, ça couvre toute la pièce. Bon investissement." },
  { n: 'Nadia', v: 'Cocody', stars: 5, t: "Avec un bébé et un chat à la maison, je ne voulais plus d'insecticide. Silencieux, aucun danger, et les moustiques ont disparu 😍" },
];

function TestimonialCard({ r }: { r: (typeof TESTIMONIALS)[number] }) {
  return (
    <div className="rounded-2xl bg-white/85 p-4 shadow-lg ring-1 ring-[#2563EB]/15">
      <div className="flex items-center gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#1E3A8A] via-[#2563EB] to-[#06B6D4] text-[15px] font-black text-white">
          {r.n[0]}
        </span>
        <div className="min-w-0 flex-1">
          <p className="text-[13px] font-black text-[#0A1F44]">{r.n} <span className="font-semibold text-neutral-400">· {r.v}</span></p>
          <span className="inline-flex items-center gap-0.5">
...
const WHATSAPP_REVIEWS = [
  { n: 'Aïssata', v: 'Marcory', t: "Sis le boîtier est top 🙏🏾 Depuis que je l'ai branché dans la chambre, plus un seul moustique la nuit. Et aucun bruit, on entend rien 😍", h: '09:42', stars: 5 },
  { n: 'Mamadou', v: 'Koumassi', t: "Branché derrière le frigo comme conseillé : les cafards ont disparu en une semaine 😩🔥 J'ai repris 2 boîtiers pour le salon et la chambre.", h: '12:15', stars: 5 },
  { n: 'Adjoua', v: 'Treichville', t: "Commandé lundi, reçu mercredi, payé à la livraison 👌🏾 Fini les sachets empoisonnés dangereux pour mes enfants.", h: '18:03', stars: 5 },
  { n: 'Fatou', v: 'San-Pédro', t: "J'en ai pris 3 pour toute la maison 🏠 Même ma voisine en veut un maintenant, je t'envoie son numéro ❤️", h: '10:27', stars: 5 },
];
...
function WhatsAppBubble({ r, i }: { r: (typeof WHATSAPP_REVIEWS)[number]; i: number }) {
  ...
  return (
    <div className={`flex ${mine ? 'justify-start' : 'justify-end'}`}>
      <div className={`relative max-w-[85%] rounded-2xl px-3.5 py-2.5 shadow-sm ${mine ? 'rounded-tl-md bg-white ring-1 ring-[#2563EB]/15' : 'rounded-tr-md bg-[#DCF8C6] ring-1 ring-emerald-600/10'}`}>
...
/* Landing                                                             */
/* ================================================================== */
export default function RepulsifUltrasonsLanding() {
  ...
  const openModal = useCallback((q?: number) => {
    // Sans quantité explicite, on ouvre toujours sur 1 boîtier (CTA collant,
  ...
  useEffect(() => {
    document.title = 'Répulsif Ultrasons Anti-Nuisibles — Moustiques, souris, cafards · -50 %';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#EFF6FF] via-[#F0F9FF] to-[#FFF7ED] pb-28 text-neutral-900">
      <style>{`
      ...
      <section className="relative overflow-hidden px-4 pb-10 pt-8">
        <div className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-[#93C5FD]/40 blur-3xl" />
        ...
        <div className="relative mx-auto max-w-[560px] text-center">
          <div className="flex flex-wrap items-center justify-center gap-2">
            <span className="inline-flex rounded-full bg-gradient-to-r from-[#1E3A8A] to-[#2563EB] px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-white">🛡️ Nouveauté 2026</span>
          ...
          <h1 className="mt-5 text-[30px] font-black leading-[1.1] text-[#0A1F44] sm:text-[38px]">
            Moustiques, souris, cafards : <span className="ru-grad">débarrassez-vous-en pour de bon.</span>
          </h1>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2 text-[11px] font-bold">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/80 px-3 py-1.5 ring-1 ring-[#F59E0B]/40">
              <Stars /> <span className="text-[#0A1F44]">4,8/5</span> <span className="text-neutral-400">(2 300+ avis)</span>
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-white/80 px-3 py-1.5 text-[#1D4ED8] ring-1 ring-[#2563EB]/30">
          ...
          <div className="mt-5 flex items-baseline justify-center gap-3">
            ...
            <span className="ru-grad text-[44px] font-black leading-none sm:text-[54px]">{fmtTotal(1)} F</span>
          </div>
          <p className="mt-1 text-[11px] font-black uppercase tracking-[0.18em] text-[#EA580C]">Paiement à la livraison 🔒</p>

          <div className="relative mx-auto mt-6 max-w-[440px]">
            <div className="absolute -inset-3 rounded-[36px] bg-gradient-to-r from-[#1E3A8A]/20 via-[#2563EB]/25 to-[#06B6D4]/20 blur-xl" />
            <div className="relative overflow-hidden rounded-[32px] shadow-2xl ring-1 ring-white/60">
              <LazyImg src={MEDIA.hero} alt="Répulsif électronique à ultrasons Pest Repeller : la solution ultime contre moustiques, souris et cafards, sans odeur et facile à brancher" aspect="1/1" priority />
            </div>
          </div>

          <div className="mx-auto mt-6 max-w-sm"><PestCTA big onClick={() => openModal()}>Commander · {fmtTotal(1)} F</PestCTA></div>
          <div className="mt-4 flex flex-wrap justify-center gap-2 text-[10.5px] font-bold text-[#1E3A8A]/80">
            {['🔇 Silencieux pour les humains', '🧒 Sans danger enfants & animaux', '🧪 Zéro produit chimique', '🔌 Branchement simple 220V'].map((b) => (
              <span key={b} className="rounded-full bg-white/70 px-3 py-1 ring-1 ring-[#2563EB]/25">{b}</span>
            ))}
          </div>
        </div>
      </section>

      <Marquee items={['Paiement à la livraison', 'Livraison rapide Abidjan & intérieur', "-50 % aujourd'hui", 'Sans produit chimique', 'Sans danger enfants & animaux']} />

      {/* ==================== COMPTE À REBOURS ==================== */}
      <section className="bg-gradient-to-r from-[#0A1533] via-[#1E3A8A] to-[#1D4ED8] px-4 py-8">
        <div className="mx-auto max-w-[560px] text-center">
          ...
          <p className="mt-3 text-[11px] font-semibold text-[#BFDBFE]">À minuit, le boîtier repasse à {fmtN(OLD_UNIT)} F. Après, il sera trop tard.</p>
        </div>
      </section>

      {/* ==================== BLOCS MÉDIAS ==================== */}
      <MediaBlock
        media={<LazyVideo src={MEDIA.demoVideo} poster={MEDIA.demoPoster} badge="Démo réelle" />}
        bg="bg-gradient-to-b from-[#0A1533] via-[#1E3A8A] to-[#1D4ED8]"
        kicker="La nuit"
        title={<>Pendant que vous dormez, <span className="text-[#67E8F9]">lui, il veille</span></>}
        text="C'est la nuit que souris et cafards sortent. Branché en continu, le boîtier émet des ultrasons qui rendent la pièce invivable pour eux — sans un bruit pour vous."
        cta="Je veux le mien"
        onCta={() => openModal()}
        dark
        ratio="9/16"
      ...
      <MediaBlock
        media={<LazyImg src={MEDIA.marreNuisibles} alt="Visuel : marre des souris et des cafards — le répulsif Pest Repeller chasse les nuisibles de la maison" aspect="1/1" />}
        bg="bg-gradient-to-b from-[#FFF7ED] to-[#FED7AA]/50"
        kicker="Ras-le-bol"
        title={<>Marre des souris <span className="text-[#EA580C]">et des cafards ?</span></>}
        text="Pièges, insecticides, sachets empoisonnés : vous avez tout essayé, ils reviennent toujours. Passez à la solution propre qui les fait fuir et les empêche de revenir."
      ...
      <MediaBlock
        media={<LazyImg src={MEDIA.cuisineGif} alt="Animation réelle : la cuisine protégée par le répulsif à ultrasons branché sur la prise" aspect="1/1" />}
        bg="bg-gradient-to-b from-[#EFF6FF] to-[#BFDBFE]/50"
        kicker="Dans la cuisine"
        title={<>Votre cuisine redevient <span className="ru-grad">interdite aux nuisibles</span></>}
        text="Les ultrasons couvrent toute la pièce : cafards et rongeurs quittent les placards, l'arrière du frigo et les recoins — et ne reviennent plus."
        cta="Profiter du -50 %"
        onCta={() => openModal()}
        ratio="1/1"
      />
      ...
      <MediaBlock
        media={<LazyImg src={MEDIA.protegerMaison} alt="Visuel : protégez votre maison — le répulsif à ultrasons repousse moustiques, souris et cafards" aspect="1080/1350" />}
        bg="bg-gradient-to-b from-[#ECFDF5] to-[#A7F3D0]/40"
        kicker="Toute la maison"
        title={<>Une protection 24h/24, <span className="ru-grad">sans rien recharger</span></>}
        text="Pas de recharge, pas de cartouche, pas d'entretien : branché une seule fois, il protège votre maison jour et nuit, toute l'année."
      ...
      <MediaBlock
        media={<LazyImg src={MEDIA.photoReelle} alt="Photo réelle : le boîtier Pest Repeller branché sur une prise murale chez un client" aspect="720/960" />}
        bg="bg-gradient-to-b from-[#0A1533] via-[#1E3A8A] to-[#155E9C]"
        kicker="Photo réelle client"
        title={<>Installé en 10 secondes, <span className="text-[#67E8F9]">sans mode d'emploi</span></>}
        text="Une simple prise 220V suffit : vous branchez, le voyant s'allume, la protection démarre aussitôt. Aucun réglage, aucune pile."
        cta="Je commande"
        onCta={() => openModal()}
        dark
        ratio="720/960"
      ...
      <MediaBlock
        media={<LazyImg src={MEDIA.priseSimple} alt="Le répulsif branché directement sur une prise murale standard" aspect="1/1" />}
        bg="bg-gradient-to-b from-[#F0F9FF] to-[#A5F3FC]/40"
        kicker="Branchement simple"
        title={<>Chaque prise devient <span className="ru-grad">un bouclier</span></>}
        text="Un boîtier par pièce pour une couverture totale : salon, chambre, cuisine, boutique ou dépôt. Branchez, et c'est réglé."
      ...
      <MediaBlock
        media={<LazyImg src={MEDIA.chaquePiece} alt="Le répulsif installé dans une pièce de la maison, discret sur la prise" aspect="1/1" />}
        bg="bg-gradient-to-b from-[#172554] via-[#1E3A8A] to-[#0E7490]"
        kicker="Sans danger"
        title={<>Silencieux pour vous, <span className="text-[#FDBA74]">insupportable pour eux</span></>}
        text="Les ultrasons sont inaudibles pour les humains et sans danger pour les enfants, les chiens et les chats. Seuls les nuisibles décampent."
      ...
      <Marquee items={['Paiement à la livraison', "-50 % ce soir seulement", 'Sans produit chimique · sans odeur', 'Livraison rapide Abidjan & intérieur', 'Protection 24h/24']} />

      {/* ==================== CHIFFRES / PREUVE SOCIALE ==================== */}
      <section className="bg-gradient-to-b from-[#EFF6FF] via-[#F0F9FF] to-[#FFF7ED] px-4 py-10">
        <div className="mx-auto grid max-w-[560px] grid-cols-3 gap-3 text-center">
          {[
            ...
            <div key={s.l} className="rounded-2xl bg-white/85 p-4 shadow-lg ring-1 ring-[#2563EB]/15">
              <span className="text-[18px]">{s.i}</span>
              <p className="ru-grad mt-1 text-[22px] font-black leading-none sm:text-[26px]">{s.v}</p>
      ...
      {/* ==================== TÉMOIGNAGES ==================== */}
      <section className="bg-gradient-to-b from-[#F0F9FF] to-[#BFDBFE]/40 px-4 py-12">
        <div className="mx-auto max-w-[560px]">
          <div className="text-center">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1.5 text-[11px] font-bold ring-1 ring-[#F59E0B]/40">
              <Stars /> <span className="text-[#0A1F44]">4,8/5 — +2 300 clients satisfaits</span>
            </span>
            <h2 className="mt-3 text-[24px] font-black text-[#0A1F44] sm:text-[28px]">Ils ont chassé les nuisibles <span className="ru-grad">de chez eux</span></h2>
          </div>
          <div className="mt-6 space-y-3">
            {TESTIMONIALS.map((r) => <TestimonialCard key={r.n} r={r} />)}
          </div>
          <div className="mx-auto mt-6 max-w-sm"><PestCTA onClick={() => openModal()}>Rejoindre les clients satisfaits</PestCTA></div>
        </div>
      </section>

      {/* ==================== TÉMOIGNAGES WHATSAPP ==================== */}
      <section className="bg-gradient-to-b from-[#E9F7EF] via-[#F0F9FF] to-[#FFF7ED] px-4 py-12">
        <div className="mx-auto max-w-[560px]">
          <div className="text-center">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/80 px-3 py-1.5 text-[11px] font-bold ring-1 ring-[#F59E0B]/40">
              <Stars /> <span className="text-[#0A1F44]">97 % de clients recommandent</span>
            </span>
            <h2 className="mt-3 text-[24px] font-black text-[#0A1F44] sm:text-[28px]">Ils nous écrivent <span className="ru-grad">sur WhatsApp</span></h2>
          </div>
          <div className="mt-6 space-y-3">
            {WHATSAPP_REVIEWS.map((r, i) => <WhatsAppBubble key={i} r={r} i={i} />)}
          </div>
          <p className="mt-4 text-center text-[11px] font-bold text-[#1D4ED8]">🔒 Paiement à la livraison · Vous ne payez qu'à la réception du boîtier</p>
          <div className="mx-auto mt-5 max-w-sm"><PestCTA onClick={() => openModal()}>Commander en toute confiance</PestCTA></div>
        </div>
      </section>

      {/* ==================== OFFRE / URGENCE ==================== */}
      <section className="bg-gradient-to-b from-[#0A1533] via-[#1E3A8A] to-[#155E9C] px-4 py-12">
        <div className="mx-auto max-w-[560px] text-center">
          ...
          <div className="mx-auto mt-6 max-w-sm"><PestCTA big onClick={() => openModal()}>J'en profite avant minuit</PestCTA></div>
        </div>
      </section>

      {/* ==================== PACKS ==================== */}
      <section className="bg-gradient-to-b from-[#EFF6FF] via-[#F0F9FF] to-[#FED7AA]/40 px-4 py-12">
        <div className="mx-auto max-w-[560px]">
          <h2 className="text-center text-[24px] font-black text-[#0A1F44] sm:text-[28px]">Choisissez votre <span className="ru-grad">pack</span></h2>
          <p className="mt-2 text-center text-[13px] text-[#1E3A8A]/75">1 pour une pièce · 2 pour salon + chambre · 3 pour toute la maison.</p>
          <div className="mt-6 space-y-3">
            {QTY_OPTS.map((o) => {
              ...
              return (
                <button key={o.v} type="button"
                  ...
                  className={`relative w-full rounded-2xl border-2 bg-white/90 p-4 text-left transition ${active ? 'scale-[1.01] border-[#F97316] shadow-xl' : 'border-[#93C5FD]/40 hover:border-[#2563EB]'}`}>
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-black text-[#0A1F44]">{o.label} <span className="ml-1 text-[13px] font-bold text-neutral-400 line-through">{fmtN(OLD_UNIT * o.v)} F</span></p>
          ...
          <div className="mx-auto mt-6 max-w-sm">
            <PestCTA big onClick={() => openModal(selectedPack)}>Commander · {fmtTotal(selectedPack)} F</PestCTA>
          </div>
          <p className="mt-6 text-center text-[15px] font-black text-[#0A1F44]">
            Moustiques. Souris. Cafards. <span className="ru-grad">Dehors, pour de bon.</span>
          </p>
          <div className="mx-auto mt-4 max-w-sm"><PestCTA onClick={() => openModal()}>Dernier clic avant minuit ⏳</PestCTA></div>
        </div>
      </section>

      {/* ==================== GARANTIES COD ==================== */}
      <section className="bg-gradient-to-b from-[#E9F7EF] to-[#F0F9FF] px-4 py-10">
        <div className="mx-auto grid max-w-[560px] grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            { icon: '💵', t: 'Paiement à la livraison', d: 'Vous payez uniquement à la réception du boîtier.' },
            ...
            <div key={g.t} className="rounded-2xl bg-white/80 p-4 text-center ring-1 ring-[#93C5FD]/30">
              ...
              <p className="mt-2 text-[12px] font-black text-[#0A1F44]">{g.t}</p>
      ...
      {/* ==================== FOOTER COD ==================== */}
      <footer className="bg-gradient-to-b from-[#0A1533] to-[#172554] px-4 pb-8 pt-6 text-center text-[10.5px] text-white/60">
        ...
        <p className="mt-3">© {new Date().getFullYear()} · Répulsif Ultrasons Anti-Nuisibles · Côte d'Ivoire</p>
      </footer>
