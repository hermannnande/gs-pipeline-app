/**
 * Page merci — Lunettes Vision Nocturne (slug lunette-de-nuit).
 */
...
const TEMPLATE_SLUG = 'lunette-de-nuit';
...
export default function LunetteDeNuitThankYou() {
  ...
  useEffect(() => { document.title = 'Merci — Lunettes Vision Nocturne'; }, []);

  useEffect(() => {
    if (fired.current) return;
    fired.current = true;
    const sk = ref ? `lvn_purchase_${ref}` : '';
    if (sk && sessionStorage.getItem(sk)) return;
    const eventId = ref ? `purchase_${ref}` : `purchase_${Date.now()}`;
    const fire = () => {
      initPixel(META_PIXEL_ID);
      window.fbq?.('track', 'Purchase', {
        content_name: 'Lunettes Vision Nocturne', content_ids: [PRODUCT_CODE], content_type: 'product',
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-sky-50 via-white to-yellow-50 px-4 py-10">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-xl">
        <div className="bg-gradient-to-br from-sky-600 via-emerald-500 to-pink-500 px-6 py-8 text-center text-white">
          ...
          <p className="mt-1.5 text-[13px] text-sky-100">Vos lunettes vision nocturne sont bien enregistrées.</p>
        </div>
        <div className="space-y-4 p-5">
          {ref && (
            <div className="rounded-xl bg-yellow-50 p-3 text-center ring-1 ring-yellow-100">
              <p className="text-[10px] font-bold uppercase text-yellow-700">Référence</p>
          ...
          <Link to="/lunette-de-nuit" className="block rounded-full bg-gradient-to-r from-sky-500 to-emerald-500 py-3 text-center text-[12px] font-black uppercase text-white">Retour au produit</Link>
        </div>
      </div>
    </div>
  );
