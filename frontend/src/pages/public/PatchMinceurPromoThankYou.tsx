/**
 * Page merci — Patch Minceur Promo (slug patch-minceur-promo).
 * Produit PATCH_MINCEUR_PROMO · Pixel Meta : 924252450279192
 */
...
const TEMPLATE_SLUG = 'patch-minceur-promo';
const PRODUCT_CODE = 'PATCH_MINCEUR_PROMO';
const PRICES: Record<number, number> = { 1: 7500, 2: 14000, 3: 20000 };
...
export default function PatchMinceurPromoThankYou() {
  ...
  useEffect(() => { document.title = 'Merci — Patch Minceur Promo'; }, []);

  useEffect(() => {
    if (fired.current) return;
    fired.current = true;
    const sk = ref ? `pmp_purchase_${ref}` : '';
    if (sk && sessionStorage.getItem(sk)) return;
    const eventId = ref ? `purchase_${ref}` : `purchase_${Date.now()}`;
    const fire = () => {
      initPixel(META_PIXEL_ID);
      window.fbq?.('track', 'Purchase', {
        content_name: 'Patch Minceur GLP Promo', content_ids: [PRODUCT_CODE], content_type: 'product',
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-violet-50 via-white to-fuchsia-50 px-4 py-10">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-violet-100 bg-white shadow-xl">
        <div className="bg-gradient-to-br from-violet-700 via-fuchsia-600 to-purple-700 px-6 py-8 text-center text-white">
          ...
          <p className="mt-1.5 text-[13px] text-violet-100">Votre patch minceur promo est bien enregistré.</p>
        </div>
        <div className="space-y-4 p-5">
          {ref && (
            <div className="rounded-xl bg-violet-50 p-3 text-center ring-1 ring-violet-100">
              <p className="text-[10px] font-bold uppercase text-violet-600">Référence</p>
              <p className="mt-0.5 font-mono text-sm font-bold text-violet-900">{ref.slice(0, 12).toUpperCase()}</p>
            </div>
          )}
          <p className="text-center text-[12px] text-neutral-600">
            Montant : <span className="font-bold">{fmtTotal(qty)} F</span>
          </p>
          <p className="text-center text-[12px] text-neutral-600">Un conseiller vous appelle pour confirmer et organiser la livraison.</p>
          <Link to={`/patch-minceur-promo?company=${encodeURIComponent(company)}`} className="block rounded-full bg-violet-600 py-3 text-center text-[12px] font-black uppercase text-white">Retour au produit</Link>
        </div>
      </div>
    </div>
  );
