/**
 * Modal commande — Bande sport minceur (BANDE_SPORT_MINCEUR).
 */
...
const inputCls = 'block h-12 w-full rounded-2xl border border-sky-100 bg-white px-4 text-[16px] font-medium outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-300/30';

export default function OrderModalBandeSport({ open, onClose, cfg, product, setProduct, qtyOptions, initialQty = 1 }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center sm:p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 flex w-full max-w-[420px] flex-col overflow-hidden rounded-t-[28px] bg-white shadow-2xl sm:rounded-3xl">
        <div className="border-b border-sky-50 px-5 pb-4 pt-3">
          <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-cyan-200 sm:hidden" />
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-600">Paiement à la livraison</p>
              <h3 className="mt-0.5 text-[18px] font-black">{cfg.title || 'Bande sport minceur'}</h3>
              <p className="mt-1 text-[11px] font-semibold tabular-nums text-sky-600">⏱ {pad(cd.m)}:{pad(cd.s)} · livraison rapide</p>
            </div>
            <button type="button" onClick={() => !sending && onClose()} className="flex h-9 w-9 items-center justify-center rounded-full bg-sky-50 text-cyan-600">✕</button>
          </div>
        </div>
        <form onSubmit={async (e) => { e.preventDefault(); await submit({ name, city, phone, qty }); }} className="flex flex-col gap-4 px-5 py-4">
            ...
            {qtyOptions.map((o) => {
              ...
              return (
                <button key={o.v} type="button" onClick={() => setQty(o.v)}
                  className={`relative rounded-2xl border-2 px-2 py-3 text-center transition ${active ? 'scale-[1.02] border-cyan-500 bg-gradient-to-b from-sky-50 to-emerald-50 shadow-md' : 'border-sky-100 bg-white'}`}>
                  {o.tag && active && <span className="absolute -top-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-cyan-500 px-2 py-0.5 text-[7px] font-black uppercase text-white">{o.tag}</span>}
                  <p className={`text-[15px] font-black ${active ? 'text-cyan-700' : 'text-neutral-800'}`}>{o.sub}</p>
                  <p className="mt-0.5 text-[9px] font-bold uppercase text-cyan-500">{o.label}</p>
                </button>
              );
            })}
          </div>
          <div className="space-y-3">
            ...
            <div className="flex h-12 overflow-hidden rounded-2xl border border-sky-100 bg-white">
              <span className="flex items-center border-r border-sky-100 bg-sky-50/80 px-3 text-[13px] font-bold text-cyan-700">+225</span>
          ...
          <button type="submit" disabled={sending}
            className="flex h-[54px] w-full items-center justify-center rounded-2xl bg-gradient-to-r from-sky-500 via-cyan-500 to-emerald-500 text-[15px] font-black text-white shadow-lg disabled:opacity-60">
        ...
        <p className="px-5 pb-4 text-center text-[10px] text-cyan-500" style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom, 0px))' }}>
