/**
 * Modal de commande "Spray Anti-Lipome" — v4 FULLSCREEN MOBILE + STICKY CTA.
 *
 ...
 *       1) HEADER  (flex-none) : icone + titre + countdown + stock
 *       2) BODY    (flex-1, overflow-y-auto) : qty segmented control + inputs
 *       3) FOOTER  (flex-none, sticky bottom) : recap facture + CTA TOUJOURS VISIBLE
 *
 * Design lisible, simple et elegant :
 *   - Fond blanc epure, ligne deco magenta en haut
 *   - Segmented control horizontal avec pill active magenta glissante
 *   - Inputs clairs avec label au-dessus
 *   - Recap style "facture" dans le footer (sous-total, livraison, total magenta)
 *   - CTA pleine largeur MAGENTA tres visible
 *   - Micro-countdown discret avec icone horloge
 *
  ...
  return (
    ...
    >
      ...
      {/*
        ...
        - Desktop : max-h-[92vh] + max-w-[420px] + rounded-[24px]

        flex flex-col -> 3 zones empilees (header / body / footer)
        Le footer reste TOUJOURS visible grace au flex-1 du body qui scroll.
      */}
      <div className="slm-shell relative z-10 flex h-[100svh] w-full flex-col overflow-hidden bg-white shadow-2xl animate-[slmslide_.25s_cubic-bezier(.22,.8,.4,1)] sm:h-auto sm:max-h-[92vh] sm:max-w-[420px] sm:rounded-[24px]">

        {/* Fine ligne deco magenta en haut */}
        <div className="h-1 w-full flex-none bg-gradient-to-r from-purple-500 via-fuchsia-500 to-pink-500"/>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div>
              ...
              <input
                ...
                autoComplete="name"
                required
                className="block h-11 w-full rounded-xl border border-neutral-200 bg-white px-3 text-[15px] sm:text-[14px] font-medium text-neutral-900 outline-none transition placeholder:text-neutral-400 focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-500/20"
              />
            </div>

            <div>
              <label htmlFor="slm-phone" className="mb-0.5 block text-[10px] font-bold text-neutral-500">Telephone</label>
              <div className="flex h-11 overflow-hidden rounded-xl border border-neutral-200 bg-white transition focus-within:border-fuchsia-500 focus-within:ring-2 focus-within:ring-fuchsia-500/20">
        ...
        */}
        ...
        >
          {/* Recap style facture */}
          <div className="mb-2.5 rounded-xl bg-neutral-50 p-3 ring-1 ring-neutral-100">
            ...
            <div className="flex items-baseline justify-between">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="bg-gradient-to-r from-fuchsia-600 via-pink-600 to-rose-600 bg-clip-text text-[20px] font-black tabular-nums text-transparent">
                  {fmt(total)}
                </span>
              </div>
            </div>
      ...
      <style>{`
        ...
        @supports (height: 100svh) {
          .slm-shell { height: 100svh; }
        }
        @media (min-width: 640px) {
          .slm-shell { height: auto !important; }
        }
        @media (max-width: 639px) {
          .slm-shell input:focus,
          .slm-shell textarea:focus { font-size: 16px !important; }
        }
      `}</style>
    </div>
  );
