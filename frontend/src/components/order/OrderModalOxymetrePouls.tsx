/**
 * Modal commande — Oxymètre de Pouls Digital (OXYMETRE_POULS).
 * Base : OrderModalSangleRotulienne (logique metier useOrderSubmit 100% conservee).
 * Pas de selection de coloris : un seul produit, packs 1/2/3 oxymètres.
 *
 * Theme : bleu medical #1D4ED8 / cyan #06B6D4 + rouge coeur #EF4444.
 */
...
const inputCls = 'block h-12 w-full rounded-2xl border border-[#93C5FD]/50 bg-white px-4 text-[16px] font-medium outline-none focus:border-[#06B6D4] focus:ring-2 focus:ring-[#06B6D4]/25';

export default function OrderModalOxymetrePouls({ open, onClose, cfg, product, setProduct, qtyOptions, initialQty = 1 }: Props) {
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center sm:p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 flex max-h-[94vh] w-full max-w-[420px] flex-col overflow-hidden rounded-t-[28px] bg-gradient-to-b from-[#EFF6FF] to-white shadow-2xl sm:rounded-3xl">
        <div className="shrink-0 border-b border-[#93C5FD]/30 px-5 pb-4 pt-3">
          <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-[#1D4ED8]/40 sm:hidden" />
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#1D4ED8]">Paiement à la livraison</p>
              <h3 className="mt-0.5 text-[18px] font-black text-[#0B1E4B]">{cfg.title || 'Oxymètre de Pouls Digital'}</h3>
              <p className="mt-1 text-[11px] font-semibold tabular-nums text-[#EF4444]">⏱ {pad(cd.m)}:{pad(cd.s)} · offre -50 % ce soir</p>
            </div>
        ...
        >
            ...
            {qtyOptions.map((o) => {
              ...
              return (
                <button key={o.v} type="button" onClick={() => setQty(o.v)}
                  className={`relative rounded-2xl border-2 px-2 py-3 text-center transition ${active ? 'scale-[1.02] border-[#EF4444] bg-gradient-to-b from-[#FFF1F2] to-[#EFF6FF] shadow-md' : 'border-[#93C5FD]/40 bg-white'}`}>
                  {o.tag && active && <span className="absolute -top-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-gradient-to-r from-[#EF4444] to-[#B91C1C] px-2 py-0.5 text-[7px] font-black uppercase text-white">{o.tag}</span>}
                  <p className={`text-[15px] font-black ${active ? 'text-[#EF4444]' : 'text-neutral-800'}`}>{o.sub}</p>
          ...
          <p className="-mb-1 text-center text-[10.5px] font-semibold text-[#1D4ED8]/70">🚚 Livraison à vos frais · Paiement à la réception</p>
          <button type="submit" disabled={sending}
            className="flex h-[54px] w-full items-center justify-center rounded-2xl bg-gradient-to-r from-[#1D4ED8] via-[#06B6D4] to-[#EF4444] text-[15px] font-black text-white shadow-[0_14px_36px_-10px_rgba(29,78,216,.55)] transition hover:brightness-110 active:scale-[0.99] disabled:opacity-60">
