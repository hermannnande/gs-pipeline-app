/**
 * Modal commande — Lunettes Vision Nocturne (LUNETTE_VISION_NOCTUNE).
 */
...
const inputCls = 'block h-12 w-full rounded-2xl border border-sky-100 bg-white px-4 text-[16px] font-medium text-neutral-900 outline-none transition placeholder:text-sky-300/80 focus:border-sky-400 focus:ring-2 focus:ring-sky-300/30';

export default function OrderModalLunetteDeNuit({ open, onClose, cfg, product, setProduct, qtyOptions, initialQty = 1 }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center sm:p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 flex w-full max-w-[420px] flex-col overflow-hidden rounded-t-[28px] bg-white shadow-2xl sm:rounded-3xl">
        <div className="border-b border-sky-50 bg-gradient-to-r from-sky-50 via-yellow-50/50 to-pink-50 px-5 pb-4 pt-3">
          <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-sky-200 sm:hidden" />
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-sky-600">Paiement à la livraison</p>
              <h3 className="mt-0.5 text-[18px] font-black text-neutral-900">{cfg.title || 'Lunettes Vision Nocturne'}</h3>
              <p className="mt-1 text-[11px] font-semibold tabular-nums text-emerald-600">⏱ {pad(cd.m)}:{pad(cd.s)} · offre -50% ce soir</p>
            </div>
            <button type="button" onClick={() => !sending && onClose()} aria-label="Fermer"
              className="flex h-9 w-9 items-center justify-center rounded-full bg-sky-50 text-sky-600">✕</button>
          </div>
        </div>
        <form onSubmit={async (e) => { e.preventDefault(); await submit({ name, city, phone, qty }); }} className="flex flex-col gap-4 px-5 py-4">
            ...
            {qtyOptions.map((o) => {
              ...
              return (
                <button key={o.v} type="button" onClick={() => setQty(o.v)}
                  className={`relative rounded-2xl border-2 px-2 py-3 text-center transition ${active ? 'scale-[1.02] border-sky-500 bg-gradient-to-b from-sky-50 to-yellow-50 shadow-md' : 'border-sky-100 bg-white'}`}>
                  {o.tag && active && <span className="absolute -top-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-yellow-400 px-2 py-0.5 text-[7px] font-black uppercase text-neutral-900">{o.tag}</span>}
                  <p className={`text-[15px] font-black ${active ? 'text-sky-700' : 'text-neutral-800'}`}>{o.sub}</p>
                  <p className="mt-0.5 text-[9px] font-bold uppercase text-sky-500">{o.label}</p>
                </button>
              );
            })}
          </div>
          <div className="space-y-3">
            ...
            <div className="flex h-12 overflow-hidden rounded-2xl border border-sky-100 bg-white">
              <span className="flex items-center border-r border-sky-100 bg-sky-50/80 px-3 text-[13px] font-bold text-sky-700">+225</span>
          ...
          <button type="submit" disabled={sending}
            className="flex h-[54px] w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-sky-500 via-emerald-500 to-pink-500 text-[15px] font-black text-white shadow-lg disabled:opacity-60">
        ...
        <p className="px-5 pb-4 text-center text-[10px] text-sky-500" style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom, 0px))' }}>
