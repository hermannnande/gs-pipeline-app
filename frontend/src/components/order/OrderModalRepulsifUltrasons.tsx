/**
 * Modal commande — Répulsif Ultrasons Anti-Nuisibles (REPULSIF_ULTRASONS).
 * Base : OrderModalSupportTelephone (logique metier useOrderSubmit 100% conservee).
 * Pas de selection de coloris : un seul produit, packs 1/2/3 boîtiers.
 *
 * Theme : bleu nuit #1E3A8A / bleu #2563EB / cyan #06B6D4 + accent orange #F97316.
 */
...
const inputCls = 'block h-12 w-full rounded-2xl border border-[#93C5FD]/50 bg-white px-4 text-[16px] font-medium outline-none focus:border-[#F97316] focus:ring-2 focus:ring-[#F97316]/25';

export default function OrderModalRepulsifUltrasons({ open, onClose, cfg, product, setProduct, qtyOptions, initialQty = 1 }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center sm:p-4" role="dialog" aria-modal="true">
      <div onClick={() => !sending && onClose()} className="absolute inset-0 bg-[#0A1533]/60 backdrop-blur-[2px]" />
      <div className="relative z-10 flex max-h-[94vh] w-full max-w-[420px] flex-col overflow-hidden rounded-t-[28px] bg-gradient-to-b from-[#EFF6FF] to-white shadow-2xl sm:rounded-3xl">
        <div className="shrink-0 border-b border-[#93C5FD]/30 px-5 pb-4 pt-3">
          <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-[#2563EB]/40 sm:hidden" />
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#1D4ED8]">Paiement à la livraison</p>
              <h3 className="mt-0.5 text-[18px] font-black text-[#0A1F44]">{cfg.title || 'Répulsif Ultrasons Anti-Nuisibles'}</h3>
        ...
        >
            ...
            {qtyOptions.map((o) => {
              ...
              return (
                <button key={o.v} type="button" onClick={() => setQty(o.v)}
                  className={`relative rounded-2xl border-2 px-2 py-3 text-center transition ${active ? 'scale-[1.02] border-[#F97316] bg-gradient-to-b from-[#FFF7ED] to-[#EFF6FF] shadow-md' : 'border-[#93C5FD]/40 bg-white'}`}>
                  {o.tag && active && <span className="absolute -top-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-gradient-to-r from-[#F97316] to-[#EA580C] px-2 py-0.5 text-[7px] font-black uppercase text-white">{o.tag}</span>}
                  <p className={`text-[15px] font-black ${active ? 'text-[#EA580C]' : 'text-neutral-800'}`}>{o.sub}</p>
          ...
          <div className="space-y-3">
            ...
            <div className="flex h-12 overflow-hidden rounded-2xl border border-[#93C5FD]/50 bg-white">
              <span className="flex items-center border-r border-[#93C5FD]/40 bg-[#EFF6FF] px-3 text-[13px] font-bold text-[#1D4ED8]">+225</span>
          ...
          <button type="submit" disabled={sending}
            className="flex h-[54px] w-full items-center justify-center rounded-2xl bg-gradient-to-r from-[#FB923C] via-[#F97316] to-[#EA580C] text-[15px] font-black text-white shadow-[0_14px_36px_-10px_rgba(249,115,22,.55)] transition hover:brightness-110 active:scale-[0.99] disabled:opacity-60">
