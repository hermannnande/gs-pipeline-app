<?php
/**
 * Admin viewer — Sac Louis Vuitton (Pochette Homme Premium)
 * =========================================================
 * Accès : https://obrille.com/sac-lv/view.php?key=<ADMIN_PASSWORD>
...
$totalPochettes = array_sum(array_map(fn($o) => (int)($o['quantity'] ?? 0), $orders));
$totalRevenue   = array_sum(array_map(fn($o) => (int)($o['total']    ?? 0), $orders));

$colorBadge = function (string $c): string {
    $c = strtolower($c);
    if ($c === 'marron') return '<span class="color-badge color-marron">🤎 Marron</span>';
    if ($c === 'noir')   return '<span class="color-badge color-noir">🖤 Noir</span>';
    return htmlspecialchars($c, ENT_QUOTES);
};
?>
<!doctype html>
<html lang="fr">
...
<title>Commandes · Sac Louis Vuitton</title>
<style>
...
.color-badge { padding: 3px 10px; border-radius: 999px; font-weight: 700; font-size: 12px; display: inline-block; white-space: nowrap; }
.color-marron { background: #f3e5d8; color: #7b4b2a; border: 1px solid #d8bfa5; }
.color-noir { background: #1a1a1a; color: #f0d98c; border: 1px solid #444; }
...
<h1>Commandes · Sac Louis Vuitton <span class="badge"><?= $totalOrders ?> cmd</span></h1>
...
<div class="stats">
  ...
  <div class="stat">
    <div class="stat-label">Pochettes vendues</div>
    <div class="stat-value"><?= $totalPochettes ?> <span class="unit">pochettes</span></div>
  </div>
  <div class="stat">
  ...
  <table>
    ...
    <?php foreach ($orders as $o): ?>
      <tr>
        ...
        <td><?= $colorBadge((string)($o['color'] ?? '')) ?></td>
        <td><span class="qty">× <?= (int)($o['quantity'] ?? 0) ?></span></td>
        <td class="total"><?= number_format((int)($o['total'] ?? 0), 0, ',', ' ') ?> F</td>
        <td>
          <a class="wa-btn" target="_blank" rel="noopener" href="https://wa.me/225<?= htmlspecialchars((string)($o['phone'] ?? ''), ENT_QUOTES) ?>?text=<?= urlencode('Bonjour ' . ($o['name'] ?? '') . ', merci pour votre commande de la pochette homme premium (' . ($o['color'] ?? '') . ') chez Obrille. Nous vous confirmons votre commande.') ?>">💬 WhatsApp</a>
        </td>
      </tr>
    <?php endforeach; ?>
    </tbody>
