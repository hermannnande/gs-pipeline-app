<?php
/**
 * Endpoint de commande — Sac Louis Vuitton (Pochette Homme Premium)
 * =================================================================
 * - Accepte POST JSON ou form-urlencoded avec: name, city, phone, quantity, color
 * - color : 'marron' | 'noir' (OBLIGATOIRE, validé)
 ...
 *
 * Placer ce fichier sous /web/sac-lv/order.php
 */
...
if (!in_array($color, ['marron', 'noir'], true)) $errors[] = 'color';
...
$UNIT_PRICE  = 9900;
$colorLabel  = ucfirst($color); // Marron | Noir
$productName = 'Sac Louis Vuitton (' . $colorLabel . ')';
$order = [
...
$GOOGLE_SHEET_URL   = '';
$ADMIN_PASSWORD     = '';
$META_PIXEL_ID      = '';   // vide : CAPI désactivé tant que le client ne fournit pas le pixel
$META_CAPI_TOKEN    = '';
...
if (!empty($TELEGRAM_BOT_TOKEN) && !empty($TELEGRAM_CHAT_ID)) {
    ...
    $msg .= "Sac Louis Vuitton (Pochette Homme Premium)\n\n";
    $msg .= "👤 " . $order['name']  . "\n";
    $msg .= "📍 " . $order['city']  . "\n";
    $msg .= "📞 +225 " . $order['phone'] . "\n\n";
    $msg .= "🎨 Couleur  : *" . $colorLabel . "*\n";
    $msg .= "📦 Quantité : *" . $order['quantity'] . "* pochette(s)\n";
...
$productLabel = trim((string)($input['product'] ?? ''));
if ($productLabel === '') $productLabel = $productName; // inclut la couleur
$productCode  = trim((string)($input['product_code'] ?? 'SAC_LOUIS_VUITTON'));
