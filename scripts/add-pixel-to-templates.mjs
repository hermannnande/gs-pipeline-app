...
const PIXEL_ID = '952340034030644';
const SLUGS = ['chaussette-compression', 'crememinceurfb'];
...
async function run() {
  ...
  for (const slug of SLUGS) {
    ...
    if (!tpl) { console.log(`"${slug}" introuvable — ignore`); continue; }
    ...
    if (putRes.ok) {
      ...
      console.error(`[ERREUR] "${slug}":`, putRes.status, await putRes.text());
    }
  }
}
