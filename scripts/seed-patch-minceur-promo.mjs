/**
 * Crée (ou met à jour) le produit PATCH_MINCEUR_PROMO + template slug patch-minceur-promo
 * (duplication promo de patch-minceur-glp, commandes séparées en obgestion).
 *
 * Prix : 7 500 / 14 000 / 20 000 FCFA (qty 1/2/3).
 * Usage : node scripts/seed-patch-minceur-promo.mjs
 */
...
const SLUG = 'patch-minceur-promo';
const CODE = 'PATCH_MINCEUR_PROMO';
const SOURCE_CODE = 'PATCH_MINCEUR_GLP';
const NOM = 'Patch Minceur GLP (Promo)';
const PRICES = { prixUnitaire: 7500, prix2Unites: 14000, prix3Unites: 20000 };

const CONFIG = {
  productCode: CODE,
  templateVersion: 1,
  title: 'Patch Minceur GLP',
  thankYouUrl: `/${SLUG}/merci`,
  prices: { 1: 7500, 2: 14000, 3: 20000 },
  metaPixelId: '924252450279192',
};
...
if (product) {
  ...
  const created = await api('/products', {
    ...
    body: JSON.stringify({
      code: CODE, nom: NOM,
      description: source?.description || 'Patch minceur GLP — offre promo.',
...
const tplBody = {
  nom: NOM, slug: SLUG,
  description: 'Landing promo patch minceur GLP (duplication patch-minceur-glp)',
