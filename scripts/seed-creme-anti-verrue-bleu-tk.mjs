/**
 * Crée (ou met à jour) le produit CREME_ANTI_VERRUE_BLEU_TK + le template slug
 * creme-anti-verrue-bleu-tk (duplication TikTok Ads de creme-anti-verrue-bleu).
 *
 * Prix : 9 900 / 16 900 / 24 900 FCFA (qty 1/2/3).
 * Usage : node scripts/seed-creme-anti-verrue-bleu-tk.mjs
 */
...
const CODE = 'CREME_ANTI_VERRUE_BLEU_TK';
const SOURCE_CODE = 'CREME_ANTI_VERRUE_BLEU';
const NOM = 'Crème Anti-Verrues Bleu (TikTok)';
...
if (product) {
  ...
  const created = await api('/products', {
    ...
    body: JSON.stringify({
      code: CODE, nom: NOM,
      description: source?.description || 'Crème botanique anti-verrues — canal TikTok Ads.',
...
const tplBody = {
  nom: NOM, slug: SLUG,
  description: 'Landing TikTok Ads — duplication creme-anti-verrue-bleu (soindemoi.net)',
