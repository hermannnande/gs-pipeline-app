/**
 * Remet les prix standard 9 900 / 16 900 / 24 900 F sur tous les produits
 * dont le tarif avait ete reduit.
 *
 * Usage : node scripts/update-all-standard-prices.mjs
 */
...
const all = await loadProducts(token);
let ok = true;
...
for (const code of PRODUCTS) {
  ...
  if (!upd.ok) {
    console.error(`✗ PUT ${code} echoue : ${await upd.text()}`);
    ok = false;
    continue;
  }

  const verif = (await loadProducts(token)).find((x) => x.code?.toUpperCase() === code);
